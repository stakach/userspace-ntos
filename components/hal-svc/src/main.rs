//! `ntos-hal-svc` — the HAL service ISOLATED over SURT (spec Milestone 11.4).
//!
//! A broker root task spawns two fully-isolated children:
//!   * a **Driver Host** that loads the real `MmioInterruptTest.sys` and whose HAL
//!     exports (`MmMapIoSpace`, `IoConnectInterrupt`, …) round-trip to the service;
//!   * a **HAL service** hosting the Resource Manager + simulated device, which
//!     validates every request against a static fixture.
//!
//! They share a SURT ring pair (HAL requests → responses) and a **shared MMIO
//! frame** — the register bank both map (the driver's register access is inlined, so
//! it dereferences the mapping directly; the service asserts the device's interrupt
//! line in the same frame). Injection returns opaque ISR tokens the Driver Host runs
//! locally — the service never calls driver code (spec §11.1, §16.3).
...
pub const STATE_VADDR: u64 = 0x0000_0100_0054_0000;
/// The shared MMIO register bank — mapped into **both** children at this vaddr.
pub const HAL_MMIO_VADDR: u64 = 0x0000_0100_0055_0000;
pub const STACK_BASE: u64 = 0x0000_0100_005C_0000;
pub const IPCBUF_VADDR: u64 = 0x0000_0100_005F_B000;
pub const SCRATCH_VADDR: u64 = 0x0000_0100_005F_C000;
...
unsafe fn build_component_vspace(
    ...
    if vspace_assign_asid(pml4) != 0 {
        panic!("HAL component VSpace ASID assignment failed");
    }
    let pdpt = alloc_slot();
    let _ = untyped_retype(CAP_INIT_UNTYPED, OBJ_X86_PDPT, PAGING_BITS, 1, pdpt);
    let pd = alloc_slot();
    ...
    // Shared SURT rings + data frames + the shared MMIO register bank — RW+NX,
    // same vaddrs in both children.
    ...
    let _ = page_map(mmio, HAL_MMIO_VADDR, RW_NX, pml4);

    // Per-child private state page (RW+NX).
...
unsafe fn spawn_component(
    ...
    let pml4 = build_component_vspace(sub, comp, req, rep, mmio, with_driver);
...
unsafe extern "C" fn _start(bootinfo: *const BootInfo) -> ! {
    ...
    print_str(b"[ntos-hal] MmioInterruptTest.sys + HAL service, isolated over SURT\n");
    ...
    let f_mmio = alloc_frame(); // the shared register bank
    ...
    let f_mmio_c = copy_cap(f_mmio); // second cap → same frame, shared mapping

    // Driver Host (client): loads the real driver, produces HAL requests on the sub
    // ring, consumes responses on the comp ring, reports the verdict on RESULT.
    spawn_component(
        driver_host::driver_host_entry,
        &[(CT_N_SUB, n_sub), (CT_N_COMP, n_comp), (CT_RESULT, copy_cap(result_ep))],
        f_sub,
        f_comp,
        f_req,
        f_rep,
    ...
    // HAL service (server): Resource Manager + simulated device; consumes requests,
    // produces responses, owns the shared register bank.
    spawn_component(
        hal_service::hal_service_entry,
