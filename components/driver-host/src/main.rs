//! `ntos-driver-host` — the NT I/O Manager dispatching to an ISOLATED driver peer.
//!
...
unsafe fn build_component_vspace(sub: u64, comp: u64, req: u64, rep: u64) -> u64 {
    ...
    if vspace_assign_asid(pml4) != 0 {
        panic!("driver-host VSpace ASID assignment failed");
    }
    let pdpt = alloc_slot();
...
unsafe extern "C" fn _start(bootinfo: *const BootInfo) -> ! {
    ...
    print_str(b"[ntos-dh] NT I/O Manager - dispatch to isolated driver peer over SURT\n");
    ...
    print_str(b"[ntos-dh summary: ");
    print_u64(verdict);
    print_str(b" passed, ");
    print_u64(6u64.saturating_sub(verdict)); // 6 io-side checks
    print_str(b" failed]\n");
