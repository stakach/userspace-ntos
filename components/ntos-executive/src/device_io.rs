...
pub(crate) unsafe fn io_in16(ioport: u64, port: u16) -> u16 {
    io_in16_r(ioport, port).0
}

/// `in ax, dx` via an I/O-port cap. Returns `(value, kernel invocation label)`.
pub(crate) unsafe fn io_in16_r(ioport: u64, port: u16) -> (u16, u64) {
    let value: u64;
    let reply: u64;
    core::arch::asm!(
        "syscall",
