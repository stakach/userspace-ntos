//! Private CSR API worker transport plus ordinary hosted-thread spawn helpers.
#![allow(clippy::all)]
use crate::*;
...
/// The dedicated CSR API worker uses our native seL4-Call ntdll transport.
...
unsafe fn dispatch_hosted_server_native_service(
    ...
    for index in 4..argument_count {
        ...
        let mut bytes = [0u8; 8];
        if !csr_thread_stack_copyin(address, &mut bytes) {
            return Some(HostedWorkerNativeResult {
                service: entry.service,
                status: 0xC000_0005,
            });
    ...
    let memory = SyscallUserMemory::CsrProcess;
...
/// Spawn csrss's real `CsrApiRequestThread` as a second thread in csrss's VSpace. It faults to
/// `CSR_FAULT_EP` (no standing receiver) so it parks on its
...
pub(crate) fn csr_thread_stack_has_range(va: u64, len: usize) -> bool {
    csr_stack_has_range(va, len)
}

pub(crate) unsafe fn csr_thread_stack_copyin(va: u64, bytes: &mut [u8]) -> bool {
    csr_stack_copyin(va, bytes)
}

pub(crate) unsafe fn csr_thread_stack_copyout(va: u64, bytes: &[u8]) -> bool {
...
/// `NtSecureConnectPort(\Windows\ApiPort)`. A nested loop on
...
pub(crate) unsafe fn csr_rendezvous(
    ...
    loop {
        ...
        // into the label-2 register-slot layout the accept body below reads:
