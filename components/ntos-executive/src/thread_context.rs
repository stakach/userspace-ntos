...
use sel4_rt::legacy_context::{
    DEBUG_MASK, DEBUG_WORDS, FX_BYTES, FX_MASK, FX_WORDS, READ_WORDS, REGISTER_WORDS, RESTART_MASK,
    WRITE_WORDS,
};
...
pub(crate) struct LegacyThreadContext {
    ...
    pub(crate) debug: [u64; DEBUG_WORDS],
}
...
pub(crate) unsafe fn write(
    ...
    if let Some(image) = &context.floating_point {
        words[0] |= FX_MASK;
        for (word, bytes) in words[1 + REGISTER_WORDS..1 + REGISTER_WORDS + FX_WORDS]
            .iter_mut()
    ...
    if let Some(debug) = &context.debug {
        words[0] |= DEBUG_MASK;
        words[1 + REGISTER_WORDS + FX_WORDS..].copy_from_slice(debug);
    }
...
impl LegacyThreadContext {
    /// General, floating-point and debug state share one target quiescence interval.
    pub(crate) unsafe fn read(tcb: u64) -> Result<Self, u64> {
        ...
        for (index, register) in context.debug.iter_mut().enumerate() {
            *register = get_recv_mr(REGISTER_WORDS + FX_WORDS + index);
        }
        Ok(context)
    }
}
