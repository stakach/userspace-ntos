...
unsafe fn component_pump_loop(
    ...
    loop {
            ...
            if crate::service_sec_image::service_provider_wait_poll(
                ...
                pump_reply_recv_into!(
                    ...
                    *reply_cap,
                    msg,
                    REQUEST_TAG_LEN,
                    crate::win32k_subsystem::W32_PROVIDER_WAIT_RESUME_LABEL
                );
                continue;
            }
            ...
            && ch.caps.kind == ReqKind::Syscall
        {
            ...
            continue;
        } else if label == crate::win32k_subsystem::W32_FILE_CREATE_LABEL
            && ch.caps.kind == ReqKind::Syscall
        {
            let reply = if shared_pump::authenticated_badge(ch, msg.badge) {
                crate::service_sec_image::service_win32k_file_create_request(
                ...
                Some(nt_io_manager::io_create_file_reply::IoCreateFileReply::Rejected {
                    status: nt_process::STATUS_INVALID_PARAMETER,
                })
            };
            ...
            && ch.caps.kind == ReqKind::Syscall
        {
            let result = if shared_pump::authenticated_badge(ch, msg.badge) {
                crate::service_sec_image::service_win32k_file_close_request(
                    ch, *reply_cap, msg.badge, msg.mi, msg.m0, [msg.m1, msg.m2, msg.m3],
                )
            } else {
                crate::hosted_routed_file_close_work::SubmitResult::Ready(
            ...
            && ch.caps.kind == ReqKind::Syscall
        {
            ...
            continue;
        } else if label == crate::win32k_subsystem::W32_ATOM_LABEL
            && ch.caps.kind == ReqKind::Syscall
        {
            let (status, atom) = unsafe {
                crate::service_sec_image::service_win32k_atom_request(
                    ch.client_pi,
            ...
            continue;
        } else if label == crate::win32k_subsystem::W32_MM_SECURE_LABEL
            && ch.caps.kind == ReqKind::Syscall
        {
            let (status, handle) = unsafe {
                crate::service_sec_image::service_win32k_mm_secure_request(
                    ch.client_pi,
                    ch.client_generation,
                    msg.m0,
                    msg.m1,
            ...
            && ch.caps.kind == ReqKind::Irp
        {
            let (status, interrupt_model) =
                crate::driver_launch::service_hosted_hal_acpi_interrupt_model(ch, *reply_cap);
            pump_reply_recv4_into!(
                ch,
                *reply_cap,
                msg,
            ...
            && ch.caps.kind == ReqKind::Irp
        {
            ...
            pump_reply_recv_into!(ch, *reply_cap, msg, 1, previous as u64);
            continue;
        } else if label == crate::driver_launch::FSD_SERVICE_QUEUE_DPC_LABEL
            && ch.caps.kind == ReqKind::Irp
        {
            ...
            && ch.caps.kind == ReqKind::Irp
        {
                ...
                && msg.m3 == 0
            {
                unsafe {
                    ...
                    } else if label == crate::driver_launch::FSD_SERVICE_FLUSH_FORWARD_LABEL {
                        crate::driver_launch::service_hosted_flush_forward(
                            ch, *reply_cap, msg.badge, msg.m0, msg.m1, msg.m2,
                        )
