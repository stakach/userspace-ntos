...
use nt_hosted_runtime::{
    ...
    ndis_protocol_characteristics_layout, plan_hosted_driver_image,
...
pub const SH_VIDEO_HW_INITIALIZE: u64 = 0x9B0; // out: captured VIDEO_HW_INITIALIZATION_DATA.HwInitialize
pub const SH_VIDEO_HW_START_IO: u64 = 0x9B8; // out: captured VIDEO_HW_INITIALIZATION_DATA.HwStartIO
pub const SH_VIDEO_HW_CONTEXT: u64 = 0x9C0; // out: VideoPortInitialize HwContext
pub const SH_VIDEO_HW_DEVICE_EXTENSION_SIZE: u64 = 0x9C8; // out: miniport extension bytes (u32)
pub const SH_VIDEO_PORT_INITIALIZED: u64 = 0x9D0; // out: 1 when VideoPortInitialize captured callbacks
...
impl<const N: usize> HostedAscii<N> {
    ...
    fn starts_with_byte(&self, b: u8) -> bool {
        self.len != 0 && self.bytes[0] == b
    }

    fn ends_with_byte(&self, b: u8) -> bool {
        self.len != 0 && self.bytes[self.len - 1] == b
    }
}
...
fn hosted_driver_key_cm_path(
    ...
    if ascii_prefix_eq_ignore_case(key, r"\Registry\") {
        ...
        if !ascii_prefix_eq_ignore_case(key, r"Class\") && !path.push_str("Class\\") {
            return None;
        }
        if !path.push_ascii(&identity.driver_key_name) {
            return None;
        }
    }
    Some(path)
...
fn cm_registry_path_from_object_attributes(
    ...
    let root = root_slot?;
    if root.kind != DriverRegistryHandleKind::CmKey || root.path.is_empty() {
        return None;
    }
    if name.is_empty() {
...
fn cm_registry_path_from_rtl_path(
    ...
    if path.starts_with_byte(b'\\') {
        let mut out = HostedAscii::<HOSTED_REGISTRY_PATH_MAX>::empty();
        return out.push_ascii(path).then_some(out);
    }

    let mut out = HostedAscii::<HOSTED_REGISTRY_PATH_MAX>::empty();
    match relative_to & 0x0000_ffff {
        0 => {}
        1 => {
            if !out.push_str(r"\Registry\Machine\System\CurrentControlSet\Services") {
                return None;
            }
        }
        2 => {
            if !out.push_str(r"\Registry\Machine\System\CurrentControlSet\Control") {
                return None;
            }
        }
        _ => return None,
    ...
    if !path.is_empty() {
        if !out.is_empty() && !out.ends_with_byte(b'\\') && !out.push_byte(b'\\') {
            return None;
        }
        if !out.push_ascii(path) {
            return None;
        }
    }
...
pub(crate) unsafe fn service_hosted_provider_callback(
    ...
    let result = match record.callback_kind {
...
unsafe fn load_hosted_dependency_images(
    ...
    let mut dep_offset = plan.private_dependency_offset;
