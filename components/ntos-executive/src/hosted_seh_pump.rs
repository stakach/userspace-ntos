...
use nt_unwind::{
    exception_walk::{
        FirstRaiseStep, HandlerContexts, HandlerInvocation, WalkOutcome, WalkStep,
    },
    ...
    fn accept_step(
        ...
        match step {
            ...
            | FirstRaiseStep::Complete(WalkOutcome::TargetReached { context, .. }) => {
                ...
                Some((SehCommand::Restore { token: self.token, context_va }, false))
            }
            FirstRaiseStep::Complete(WalkOutcome::Unhandled { .. })
            | FirstRaiseStep::Complete(WalkOutcome::SecondChance { .. }) => {
                Some((SehCommand::SecondChance { token: self.token }, false))
            }
        }
    }
}
        ...
        let command = match call {
            SehCall::Prepare { token, packet_va } => {
                ...
                let mut packet = match invocation.contexts {
                    HandlerContexts::Search { .. } => SehHandlerPacket::prepare_search(
                        &active.captured, &invocation, packet_va,
                    ),
            ...
            SehCall::HandlerResult { token, packet_va, disposition } => {
                if token != active.token {
                    return None;
                }
                let Phase::Result { mut invocation, expected, packet_va: expected_va } =
                    active.phase.take()?
                else {
                    return None;
                };
                ...
                stack::apply_handler_return(
                    channel, reply, badge, packet_va, &expected, &mut invocation, disposition,
                )?.ok()?;
            ...
            SehCall::UnwindRequest { token, target_frame, target_ip, packet_va } => {
                if token != active.token {
                    return None;
                }
                let Phase::Result { mut invocation, expected, packet_va: expected_va } =
                ...
                stack::apply_handler_return(
                    channel, reply, badge, packet_va, &expected, &mut invocation, 1,
                )?.ok()?;
                let HandlerContexts::Search { exception: context, .. } = invocation.contexts else {
                    return None;
                };
