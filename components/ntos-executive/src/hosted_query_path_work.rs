//! Retained cross-domain `IofCallDriver` work for Mup's query-path IOCTL.
//!
//! The source IRP remains in Mup's VSpace. A separate canonical File IRP runs in the provider;
//! only its exact terminal is copied back before Mup's own completion routine is invoked.

use super::*;
use crate::spawn_hosts::shared_ingress::owner::runtime;
use core::num::NonZeroU64;
use nt_io_manager::{
    detached_file_irp::{ExternalFileIrpDispatchPolicy, ExternalFileIrpRequest},
    redir_query_path::{IOCTL_REDIR_QUERY_PATH, IRP_MJ_DEVICE_CONTROL},
    retained_query_path_forward::{
        QueryPathForwardOutcome, QueryPathForwardResult, RetainedQueryPathForward,
        TerminalQueryPathForward,
    },
    DeviceControlParameters, IoParameters, StackFlags,
};
use nt_process::native_handle::{NativeHandleCaller, NativeThreadProcessReference};

const STATUS_INVALID_HANDLE_LOCAL: i32 = 0xc000_0008u32 as i32;
const STATUS_INVALID_PARAMETER_LOCAL: i32 = 0xc000_000du32 as i32;
const STATUS_INVALID_DEVICE_REQUEST_LOCAL: i32 = 0xc000_0010u32 as i32;
const STATUS_INSUFFICIENT_RESOURCES_LOCAL: i32 = 0xc000_009au32 as i32;
const STATUS_DEVICE_BUSY_LOCAL: i32 = nt_status::NtStatus::DEVICE_BUSY.raw();

struct Work {
    source: hosted_query_path_capture::CapturedSourceForward,
    source_instance: DriverInstance,
    security: hosted_source_create_security::SourceSecurityIdentity,
    provider_instance: DriverInstance,
    caller: NativeHandleCaller,
    actor: NativeThreadProcessReference,
    route: nt_component_suspension::peer_registry::PeerRoute,
    dispatch: nt_component_suspension::LaneDispatchIdentity,
    reply: u64,
    token: u64,
    graph: Option<hosted_create_security_graph::RetainedProviderCreateSecurityGraph>,
    retained: Option<RetainedQueryPathForward>,
    terminal: Option<TerminalQueryPathForward>,
    canonical_irp: Option<IrpId>,
    had_canonical_irp: bool,
    completion: Option<nt_io_manager::redir_query_path::QueryPathCompletion>,
    ack: Option<RetainedAck>,
    source_released: bool,
    initial_status: Option<i32>,
    response: [u8; 4],
    abort_pending: bool,
    cancel_requested: bool,
    reply_entered: bool,
}

struct RetainedAck {
    route: nt_component_suspension::peer_registry::PeerRoute,
    dispatch: nt_component_suspension::LaneDispatchIdentity,
    reply: u64,
    token: u64,
    reply_entered: bool,
}

static mut WORK: Vec<Option<Work>> = Vec::new();
static mut EXECUTING: Vec<(usize, DriverInstance, u64)> = Vec::new();
static NEXT_TOKEN: AtomicU64 = AtomicU64::new(1);
static CURSOR: AtomicU64 = AtomicU64::new(0);

fn status_for_capture(error: hosted_query_path_capture::CaptureError) -> i32 {
    use hosted_query_path_capture::CaptureError;
    match error {
        CaptureError::InvalidCaller
        | CaptureError::MissingSecurityContext
        | CaptureError::InvalidSourceIrp
        | CaptureError::InvalidFile => STATUS_INVALID_HANDLE_LOCAL,
        CaptureError::InvalidSourceBuffer | CaptureError::QueryPath(_) => {
            STATUS_INVALID_PARAMETER_LOCAL
        }
        CaptureError::InvalidTarget => STATUS_INVALID_DEVICE_REQUEST_LOCAL,
    }
}

fn source_work_index(source_instance: DriverInstance, source_irp_address: u64) -> Option<usize> {
    let active = unsafe { &*core::ptr::addr_of!(EXECUTING) }
        .iter()
        .find(|(_, instance, address)| {
            instance.pml4 == source_instance.pml4
                && instance.hosted_domain_id == source_instance.hosted_domain_id
                && instance.hosted_domain_cookie == source_instance.hosted_domain_cookie
                && *address == source_irp_address
        })
        .map(|(index, _, _)| *index);
    if active.is_some() {
        return active;
    }
    unsafe { &*core::ptr::addr_of!(WORK) }
        .iter()
        .position(|slot| {
            slot.as_ref().is_some_and(|work| {
                work.source_instance.pml4 == source_instance.pml4
                    && work.source_instance.exec_pool_va == source_instance.exec_pool_va
                    && work.source_instance.hosted_domain_id == source_instance.hosted_domain_id
                    && work.source_instance.hosted_domain_cookie
                        == source_instance.hosted_domain_cookie
                    && work.source.source_irp_address() == source_irp_address
            })
        })
}

/// Admission is authenticated by the current physical Call and the exact source IRP ledger.
/// `None` means the Call is retained; no second provider dispatch may be issued for it.
pub(super) unsafe fn submit(
    ch: &crate::spawn_hosts::PumpChannel,
    source_irp_address: u64,
    target_device_address: u64,
    caller_badge: u64,
    active_reply_cap: u64,
) -> Option<i32> {
    let Some((_, source_instance)) = instance_for_pump_channel(ch, active_reply_cap) else {
        return Some(STATUS_INVALID_HANDLE_LOCAL);
    };
    if hosted_driver_pump_caller_tcb(ch, active_reply_cap, caller_badge).is_none() {
        return Some(STATUS_INVALID_HANDLE_LOCAL);
    }
    if source_work_index(source_instance, source_irp_address).is_some() {
        return Some(STATUS_DEVICE_BUSY_LOCAL);
    }
    let mut source = match hosted_query_path_capture::capture(
        ch,
        active_reply_cap,
        source_irp_address,
        target_device_address,
    ) {
        Ok(source) => source,
        Err(error) => return Some(status_for_capture(error)),
    };
    let security = match hosted_source_create_security::lookup_ticket(
        source_instance,
        source.request().security,
    ) {
        Some(identity) => identity,
        None => {
            source
                .release()
                .expect("unentered query-path security rejection");
            return Some(STATUS_INVALID_HANDLE_LOCAL);
        }
    };
    let Some((target_index, _, _)) =
        hosted_driver_device_route_by_device_id(source.device_id().raw())
    else {
        source
            .release()
            .expect("unentered query-path route rejection");
        return Some(STATUS_INVALID_DEVICE_REQUEST_LOCAL);
    };
    let provider_index = hosted_provider_dispatch_route_for_instance(target_index)
        .map_or(target_index, |route| route.provider_instance);
    let Some(provider_instance) = instance(provider_index) else {
        source
            .release()
            .expect("unentered query-path provider rejection");
        return Some(STATUS_INVALID_DEVICE_REQUEST_LOCAL);
    };
    let Some(route) = runtime::channel_route(ch).ok().flatten() else {
        source
            .release()
            .expect("unentered query-path route rollback");
        return Some(STATUS_INVALID_HANDLE_LOCAL);
    };
    let Ok(dispatch) = runtime::dispatch(route) else {
        source
            .release()
            .expect("unentered query-path dispatch rollback");
        return Some(STATUS_INVALID_HANDLE_LOCAL);
    };
    let Ok(reply) = runtime::current_reply(route) else {
        source
            .release()
            .expect("unentered query-path reply rollback");
        return Some(STATUS_INVALID_HANDLE_LOCAL);
    };
    let Ok(caller) = crate::provider_registry_caller::resolve(ch) else {
        source
            .release()
            .expect("unentered query-path caller rollback");
        return Some(STATUS_INVALID_HANDLE_LOCAL);
    };
    let actor = match crate::service_sec_image::with_provider_process_manager(|pm| {
        pm.reference_native_requestor(caller)
    }) {
        Ok(actor) => actor,
        Err(status) => {
            source
                .release()
                .expect("unentered query-path actor rollback");
            return Some(status as i32);
        }
    };
    let slot = (&*core::ptr::addr_of!(WORK))
        .iter()
        .enumerate()
        .find_map(|(index, row)| {
            (row.is_none()
                && !(&*core::ptr::addr_of!(EXECUTING))
                    .iter()
                    .any(|(active, _, _)| *active == index))
            .then_some(index)
        });
    if slot.is_none()
        && (&mut *core::ptr::addr_of_mut!(WORK))
            .try_reserve(1)
            .is_err()
    {
        let mut actor = actor;
        crate::service_sec_image::with_provider_process_manager(|pm| actor.release(pm))
            .expect("unentered query-path actor release");
        source
            .release()
            .expect("unentered query-path allocation rollback");
        return Some(STATUS_INSUFFICIENT_RESOURCES_LOCAL);
    }
    let token = match NEXT_TOKEN.fetch_update(Ordering::Relaxed, Ordering::Relaxed, |next| {
        next.checked_add(1)
    }) {
        Ok(token) => token,
        Err(_) => {
            let mut actor = actor;
            crate::service_sec_image::with_provider_process_manager(|pm| actor.release(pm))
                .expect("unentered query-path actor release");
            source
                .release()
                .expect("unentered query-path token rollback");
            return Some(STATUS_INSUFFICIENT_RESOURCES_LOCAL);
        }
    };
    let work = Work {
        source,
        source_instance,
        security,
        provider_instance,
        caller,
        actor,
        route,
        dispatch,
        reply,
        token,
        graph: None,
        retained: None,
        terminal: None,
        canonical_irp: None,
        had_canonical_irp: false,
        completion: None,
        ack: None,
        source_released: false,
        initial_status: None,
        response: [0; 4],
        abort_pending: false,
        cancel_requested: false,
        reply_entered: false,
    };
    let index = if let Some(index) = slot {
        (&mut *core::ptr::addr_of_mut!(WORK))[index] = Some(work);
        index
    } else {
        let rows = &mut *core::ptr::addr_of_mut!(WORK);
        rows.push(Some(work));
        rows.len() - 1
    };
    if runtime::park_retained_service(route, token).is_err() {
        let mut work = (&mut *core::ptr::addr_of_mut!(WORK))[index]
            .take()
            .expect("unparked query-path owner");
        work.actor_release().expect("unparked query-path actor");
        work.source.release().expect("unparked query-path source");
        return Some(STATUS_INSUFFICIENT_RESOURCES_LOCAL);
    }
    None
}

impl Work {
    unsafe fn actor_release(&mut self) -> Result<(), u32> {
        crate::service_sec_image::with_provider_process_manager(|pm| self.actor.release(pm))
    }

    fn cancelled(&self) -> bool {
        unsafe {
            runtime::retained_service_cancelled(self.route, self.dispatch, self.reply, self.token)
        }
    }

    unsafe fn provider_still_exact(&self) -> bool {
        let Some((target_index, _, _)) =
            hosted_driver_device_route_by_device_id(self.source.device_id().raw())
        else {
            return false;
        };
        let provider_index = hosted_provider_dispatch_route_for_instance(target_index)
            .map_or(target_index, |route| route.provider_instance);
        instance(provider_index).is_some_and(|live| {
            live.pml4 == self.provider_instance.pml4
                && live.exec_pool_va == self.provider_instance.exec_pool_va
                && live.hosted_domain_id == self.provider_instance.hosted_domain_id
                && live.hosted_domain_cookie == self.provider_instance.hosted_domain_cookie
                && live.driver_id == self.provider_instance.driver_id
        })
    }

    unsafe fn dispatch(&mut self, handler: &mut ExecNtHandler) {
        assert!(self.graph.is_none() && self.retained.is_none());
        if let Err(status) = self.actor.validate(&handler.pm) {
            self.initial_status = Some(status as i32);
            return;
        }
        self.source
            .validate_source()
            .expect("query-path source identity");
        let graph = match hosted_create_security_graph::materialize(
            self.source_instance,
            self.security,
            self.provider_instance,
        ) {
            Ok(graph) => graph,
            Err(status) => {
                self.initial_status = Some(status);
                return;
            }
        };
        let provider_address = NonZeroU64::new(graph.io_security_context_address())
            .expect("query-path provider security address");
        let input = nt_io_manager::query_path_wire::materialize_provider_input(
            self.source.request(),
            provider_address,
        );
        self.graph = Some(graph);
        let input = match input {
            Ok(input) => input,
            Err(_) => {
                self.initial_status = Some(STATUS_INSUFFICIENT_RESOURCES_LOCAL);
                self.abort_pending = true;
                return;
            }
        };
        let mut terminal_response = Vec::new();
        if terminal_response.try_reserve_exact(4).is_err() {
            self.initial_status = Some(STATUS_INSUFFICIENT_RESOURCES_LOCAL);
            self.abort_pending = true;
            return;
        }
        terminal_response.resize(4, 0);
        if self.source.validate_source().is_err() || !self.provider_still_exact() {
            self.initial_status = Some(STATUS_INVALID_DEVICE_REQUEST_LOCAL);
            self.abort_pending = true;
            return;
        }
        let request = ExternalFileIrpRequest {
            client: nt_types::ClientId(IO_MANAGER_COMPONENT_ID),
            device_id: self.source.device_id(),
            file_id: Some(self.source.file_id()),
            user_data: 0,
            requestor_tid: u64::from(self.caller.original_thread().thread_id()),
            major: IRP_MJ_DEVICE_CONTROL,
            parameters: IoParameters::DeviceControl(DeviceControlParameters {
                ioctl_code: IOCTL_REDIR_QUERY_PATH,
                input_len: input.len() as u32,
                output_len: 4,
            }),
            stack_flags: StackFlags::empty(),
            initial_information: 0,
        };
        // `dispatch` returns Err only for a proven not-entered canonical IRP. The source
        // target is still owned here, so that path can roll back without invoking Mup.
        let result = hosted_file_owners::dispatch(
            self.caller,
            request,
            &input,
            &[0; 4],
            ExternalFileIrpDispatchPolicy::FileDeviceControl,
        );
        let result = match result {
            Ok(result) => result,
            Err(status) => {
                self.initial_status = Some(status.raw());
                self.abort_pending = true;
                return;
            }
        };
        let prepared = self
            .source
            .prepare()
            .expect("entered query-path source owner");
        let identity = prepared.identity();
        let invocation = prepared
            .begin(io_manager_mut(), identity)
            .expect("entered query-path forward identity");
        let outcome = match result {
            hosted_file_owners::DispatchResult::Returned {
                status,
                information,
                buffers,
                ..
            } => {
                self.initial_status = Some(status.raw());
                if status.is_success() {
                    self.response.copy_from_slice(&buffers.output()[..4]);
                    terminal_response.copy_from_slice(&self.response);
                }
                QueryPathForwardOutcome::Returned {
                    status: status.raw() as u32,
                    information,
                    response: terminal_response,
                }
            }
            hosted_file_owners::DispatchResult::Outstanding { irp_id } => {
                self.initial_status = Some(STATUS_PENDING as i32);
                self.canonical_irp = Some(irp_id);
                self.had_canonical_irp = true;
                QueryPathForwardOutcome::Pending
            }
        };
        match invocation
            .returned(outcome)
            .finish(io_manager_mut(), identity)
        {
            QueryPathForwardResult::Terminal(terminal) => self.terminal = Some(terminal),
            QueryPathForwardResult::Retained(retained) => self.retained = Some(retained),
            QueryPathForwardResult::Rejected { error, retained } => {
                self.reconcile_rejected_completion(error, retained);
            }
        }
    }

    unsafe fn reconcile_rejected_completion(
        &mut self,
        error: nt_io_manager::retained_query_path_forward::QueryPathForwardError,
        retained: RetainedQueryPathForward,
    ) {
        use nt_io_manager::retained_query_path_forward::QueryPathForwardError;
        // The provider has completed; a malformed result is a terminal local protocol
        // failure, never a dispatch retry. This also covers synchronous returns with no IRP.
        if matches!(error, QueryPathForwardError::Completion(_)) {
            let identity = retained.identity();
            match retained.complete(
                io_manager_mut(),
                identity,
                STATUS_INVALID_DEVICE_REQUEST_LOCAL as u32,
                0,
                &[],
            ) {
                QueryPathForwardResult::Terminal(terminal) => {
                    self.initial_status = Some(STATUS_INVALID_DEVICE_REQUEST_LOCAL);
                    self.terminal = Some(terminal);
                    return;
                }
                QueryPathForwardResult::Retained(retained)
                | QueryPathForwardResult::Rejected { retained, .. } => {
                    self.retained = Some(retained);
                }
            }
        } else {
            self.retained = Some(retained);
        }
        // An identity mismatch after provider entry is indeterminate. Keep every owner, but
        // name the failure instead of silently treating it as an ordinary pending IRP.
        panic!("completed query-path result has no reconcilable target identity");
    }

    unsafe fn advance_ack(&mut self) -> bool {
        let ack = self.ack.as_ref().expect("retained query-path ACK");
        if ack.reply_entered {
            let acknowledged = runtime::reconcile_retained_service_reply(
                ack.route,
                ack.dispatch,
                ack.reply,
                ack.token,
            )
            .expect("query-path ACK Reply identity");
            if !acknowledged {
                return false;
            }
            runtime::retire_stopped_acknowledged_retained_service(
                ack.route,
                ack.dispatch,
                ack.reply,
                ack.token,
            )
            .expect("query-path ACK Reply retirement");
            let completion = self.completion.expect("retired query-path terminal");
            print_str(b"[mup-query-path] source-ticket=");
            print_hex64(self.source.source_ticket().id.get());
            print_str(b" target-device=");
            print_hex64(self.source.device_id().raw());
            print_str(b" status=");
            print_hex(completion.status);
            print_str(b" accepted-bytes=");
            print_hex(completion.length_accepted);
            print_str(b" response-u32=");
            print_hex(u32::from_le_bytes(self.response));
            print_str(b" callback-deferred-free-request=1 target-retired=1 canonical-pending-ack=");
            print_hex(u32::from(self.had_canonical_irp));
            print_str(b"\n");
            return true;
        }
        if let Some(terminal) = self.terminal.take() {
            match self.source.retire_target_after_source_completion(terminal) {
                Ok(completion) => self.completion = Some(completion),
                Err((_, terminal)) => {
                    self.terminal = Some(terminal);
                    return false;
                }
            }
        }
        if let Some(graph) = self.graph.take() {
            if let Err((_, graph)) = graph.retire() {
                self.graph = Some(graph);
                return false;
            }
        }
        if let Some(irp) = self.canonical_irp {
            if acknowledge_completed_irp(irp.raw()).is_err() {
                return false;
            }
            self.canonical_irp = None;
        }
        if !self.source_released {
            if self.source.release().is_err() {
                return false;
            }
            self.source_released = true;
        }
        if self.actor.is_held() && self.actor_release().is_err() {
            return false;
        }
        let ack = self.ack.as_mut().expect("retained query-path ACK");
        ack.reply_entered = true;
        let _ = runtime::wake_query_path_service(ack.route, ack.dispatch, ack.reply, ack.token, 0);
        false
    }

    unsafe fn advance_stopped_source(&mut self) -> bool {
        assert!(!self.reply_entered && self.ack.is_none());
        if !self.cancel_requested {
            if let Some(irp) = self.canonical_irp {
                let _ = cancel_irp_if_pending(irp.raw());
            }
            self.cancel_requested = true;
        }
        if self.retained.is_some() {
            self.poll_provider();
        }
        if self.completion.is_none() {
            let Some(terminal) = self.terminal.take() else {
                // A pending provider may finish after the source Call was stopped. Keep the
                // canonical IRP, provider graph, source pin and exact terminal owner until then.
                return false;
            };
            match self.source.retire_target_after_source_stop(terminal) {
                Ok(completion) => self.completion = Some(completion),
                Err((_, terminal)) => {
                    self.terminal = Some(terminal);
                    return false;
                }
            }
        }
        if let Some(graph) = self.graph.take() {
            if let Err((_, graph)) = graph.retire() {
                self.graph = Some(graph);
                return false;
            }
        }
        if let Some(irp) = self.canonical_irp {
            if acknowledge_completed_irp(irp.raw()).is_err() {
                return false;
            }
            self.canonical_irp = None;
        }
        if !self.source_released {
            if self.source.release().is_err() {
                return false;
            }
            self.source_released = true;
        }
        if self.actor.is_held() && self.actor_release().is_err() {
            return false;
        }
        runtime::acknowledge_retained_service_cancellation(
            self.route,
            self.dispatch,
            self.reply,
            self.token,
        )
        .expect("stopped query-path Call retirement");
        true
    }

    unsafe fn poll_provider(&mut self) {
        let Some(irp) = self.canonical_irp else {
            return;
        };
        let Some(completion) = completed_irp_exact(irp.raw()) else {
            return;
        };
        assert_eq!(completion.file_id, self.source.file_id().raw());
        assert_eq!(
            completion.requestor_tid,
            u64::from(self.caller.original_thread().thread_id())
        );
        assert_eq!(completion.major, IRP_MJ_DEVICE_CONTROL);
        let response = if (completion.status as i32) >= 0 {
            assert_eq!(
                copy_completed_irp_output_exact(irp.raw(), 0, &mut self.response),
                Ok(4)
            );
            &self.response[..]
        } else {
            &[][..]
        };
        let retained = self
            .retained
            .take()
            .expect("pending query-path forward owner");
        let identity = retained.identity();
        match retained.complete(
            io_manager_mut(),
            identity,
            completion.status,
            completion.information,
            response,
        ) {
            QueryPathForwardResult::Terminal(terminal) => self.terminal = Some(terminal),
            QueryPathForwardResult::Retained(retained) => self.retained = Some(retained),
            QueryPathForwardResult::Rejected { error, retained } => {
                self.reconcile_rejected_completion(error, retained);
            }
        }
    }

    unsafe fn publish_source(&mut self) {
        let completion = self
            .terminal
            .as_ref()
            .expect("query-path provider terminal")
            .completion();
        if (completion.status as i32) >= 0 {
            let output = self
                .source
                .source_output_exec()
                .expect("live Mup query-path output");
            core::ptr::copy_nonoverlapping(self.response.as_ptr(), output as *mut u8, 4);
        }
        let irp = self
            .source
            .source_irp_exec()
            .expect("live Mup query-path IRP");
        write_unaligned(
            (irp + WDM_X64_IRP_IO_STATUS_STATUS_OFFSET) as *mut u32,
            completion.status,
        );
        write_unaligned(
            (irp + WDM_X64_IRP_IO_STATUS_INFORMATION_OFFSET) as *mut u64,
            completion.information,
        );
        self.source
            .arm_callback_free()
            .expect("Mup deferred IRP free admission");
    }

    unsafe fn advance(&mut self, handler: &mut ExecNtHandler) -> bool {
        if self.ack.is_some() {
            return self.advance_ack();
        }
        if self.reply_entered {
            let acked = runtime::reconcile_retained_service_reply(
                self.route,
                self.dispatch,
                self.reply,
                self.token,
            )
            .expect("query-path Reply identity");
            if !acked || self.terminal.is_some() {
                return false;
            }
            runtime::retire_stopped_acknowledged_retained_service(
                self.route,
                self.dispatch,
                self.reply,
                self.token,
            )
            .expect("rejected query-path service retirement");
            self.source
                .release()
                .expect("rejected query-path source release");
            self.actor_release()
                .expect("rejected query-path actor release");
            return true;
        }
        if self.abort_pending {
            let graph = self.graph.take().expect("pre-entry query-path graph owner");
            if let Err((_status, graph)) = graph.abort_unentered() {
                self.graph = Some(graph);
                return false;
            }
            self.abort_pending = false;
        }
        if self.cancelled() {
            if self.graph.is_none()
                && self.retained.is_none()
                && self.terminal.is_none()
                && self.canonical_irp.is_none()
            {
                runtime::acknowledge_retained_service_cancellation(
                    self.route,
                    self.dispatch,
                    self.reply,
                    self.token,
                )
                .expect("unentered query-path cancellation");
                self.source
                    .release()
                    .expect("unentered query-path cancelled source");
                self.actor_release()
                    .expect("unentered query-path cancelled actor");
                return true;
            }
            return self.advance_stopped_source();
        }
        if self.initial_status.is_none() {
            self.dispatch(handler);
        }
        if self.abort_pending {
            let graph = self.graph.take().expect("pre-entry query-path graph owner");
            if let Err((_status, graph)) = graph.abort_unentered() {
                self.graph = Some(graph);
                return false;
            }
            self.abort_pending = false;
        }
        if self.retained.is_some() {
            self.poll_provider();
        }
        if self.terminal.is_none() && self.graph.is_some() {
            return false;
        }
        if self.terminal.is_some() {
            self.publish_source();
        }
        self.reply_entered = true;
        let status = self.initial_status.expect("query-path dispatch status");
        let accepted = self.terminal.is_some();
        let _ = if accepted {
            runtime::wake_query_path_service(
                self.route,
                self.dispatch,
                self.reply,
                self.token,
                status,
            )
        } else {
            runtime::wake_query_path_rejected_service(
                self.route,
                self.dispatch,
                self.reply,
                self.token,
                status,
            )
        };
        false
    }
}

pub(super) unsafe fn redrive(handler: &mut ExecNtHandler) {
    let _durable = crate::allocator::enter_durable();
    let count = (&*core::ptr::addr_of!(WORK)).len();
    if count == 0 {
        return;
    }
    let start = CURSOR.load(Ordering::Relaxed) as usize % count;
    let Some((index, mut work)) = (0..count).find_map(|step| {
        let index = (start + step) % count;
        if (&*core::ptr::addr_of!(EXECUTING))
            .iter()
            .any(|(active, _, _)| *active == index)
        {
            return None;
        }
        (&mut *core::ptr::addr_of_mut!(WORK))[index]
            .take()
            .map(|work| (index, work))
    }) else {
        return;
    };
    if (&mut *core::ptr::addr_of_mut!(EXECUTING))
        .try_reserve(1)
        .is_err()
    {
        (&mut *core::ptr::addr_of_mut!(WORK))[index] = Some(work);
        return;
    }
    (&mut *core::ptr::addr_of_mut!(EXECUTING)).push((
        index,
        work.source_instance,
        work.source.source_irp_address(),
    ));
    CURSOR.store(index as u64 + 1, Ordering::Relaxed);
    let done = work.advance(handler);
    if !done {
        (&mut *core::ptr::addr_of_mut!(WORK))[index] = Some(work);
    }
    assert_eq!(
        (&mut *core::ptr::addr_of_mut!(EXECUTING))
            .pop()
            .map(|row| row.0),
        Some(index)
    );
}

/// The ACK is a new authenticated source Call, not evidence inferred from the original Reply.
/// Mup's completion routine must have issued `IoFreeIrp` while the source ticket was armed.
pub(super) unsafe fn acknowledge(
    ch: &crate::spawn_hosts::PumpChannel,
    reply_cap: u64,
    caller_badge: u64,
    source_irp_address: u64,
) -> Option<i32> {
    let Some((_, source_instance)) = instance_for_pump_channel(ch, reply_cap) else {
        return Some(STATUS_INVALID_HANDLE_LOCAL);
    };
    if hosted_driver_pump_caller_tcb(ch, reply_cap, caller_badge).is_none() {
        return Some(STATUS_INVALID_HANDLE_LOCAL);
    }
    let Some(index) = source_work_index(source_instance, source_irp_address) else {
        return Some(STATUS_INVALID_HANDLE_LOCAL);
    };
    if (&*core::ptr::addr_of!(EXECUTING))
        .iter()
        .any(|(active, _, _)| *active == index)
    {
        return Some(STATUS_DEVICE_BUSY_LOCAL);
    }
    let Some(work) = (&mut *core::ptr::addr_of_mut!(WORK))[index].as_mut() else {
        return Some(STATUS_INVALID_HANDLE_LOCAL);
    };
    if crate::provider_registry_caller::resolve(ch) != Ok(work.caller) {
        return Some(STATUS_INVALID_HANDLE_LOCAL);
    }
    if work.ack.is_some()
        || !work.reply_entered
        || work.terminal.is_none()
        || !work.source.callback_requested_free()
    {
        return Some(STATUS_INVALID_DEVICE_REQUEST_LOCAL);
    }
    if !matches!(
        runtime::reconcile_retained_service_reply(
            work.route,
            work.dispatch,
            work.reply,
            work.token,
        ),
        Ok(true)
    ) {
        return Some(STATUS_DEVICE_BUSY_LOCAL);
    }
    work.source
        .validate_source()
        .expect("ACK source IRP and File identity");
    let route = match runtime::channel_route(ch) {
        Ok(Some(route)) if route == work.route => route,
        _ => return Some(STATUS_INVALID_HANDLE_LOCAL),
    };
    let dispatch = match runtime::dispatch(route) {
        Ok(dispatch) => dispatch,
        Err(_) => return Some(STATUS_INVALID_HANDLE_LOCAL),
    };
    let reply = match runtime::current_reply(route) {
        Ok(reply) => reply,
        Err(_) => return Some(STATUS_INVALID_HANDLE_LOCAL),
    };
    let token = match NEXT_TOKEN.fetch_update(Ordering::Relaxed, Ordering::Relaxed, |next| {
        next.checked_add(1)
    }) {
        Ok(token) => token,
        Err(_) => return Some(STATUS_INSUFFICIENT_RESOURCES_LOCAL),
    };
    // The original Call is already acknowledged. Its tombstone must retire before another
    // retained Call may occupy the same physical route.
    runtime::retire_stopped_acknowledged_retained_service(
        work.route,
        work.dispatch,
        work.reply,
        work.token,
    )
    .expect("ACK original source Reply retirement");
    runtime::park_retained_service(route, token)
        .expect("ACK retained Call admission after original retirement");
    work.ack = Some(RetainedAck {
        route,
        dispatch,
        reply,
        token,
        reply_entered: false,
    });
    None
}
