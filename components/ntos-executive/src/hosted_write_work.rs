//! Retained cross-domain WRITE forwarded by a hosted file-system driver.
//!
//! The source IRP and completion routine remain in the forwarding driver's address space.
//! Only an owned byte snapshot and canonical File identity cross into the provider domain.
...
struct Work {
    source: hosted_write_capture::CapturedSourceWrite,
    ...
    reply: u64,
    token: u64,
    retained: Option<RetainedWriteForward>,
    terminal: Option<TerminalWriteForward>,
...
fn status_for_capture(error: hosted_write_capture::CaptureError) -> i32 {
    use hosted_write_capture::CaptureError;
    match error {
...
pub(super) unsafe fn submit(
    ...
    let Some((target_index, _, _)) =
        ...
        source.release().expect("unentered WRITE route rejection");
    ...
    let Ok(reply) = runtime::current_reply(route) else {
        source.release().expect("unentered WRITE reply rollback");
        return Some(STATUS_INVALID_HANDLE_LOCAL);
    };
    let Ok(caller) = crate::provider_registry_caller::resolve(ch) else {
        source.release().expect("unentered WRITE caller rollback");
        return Some(STATUS_INVALID_HANDLE_LOCAL);
    };
    let actor = match crate::service_sec_image::with_provider_process_manager(|pm| {
        ...
        Ok(actor) => actor,
        Err(status) => {
            source.release().expect("unentered WRITE actor rollback");
    ...
    if slot.is_none() && (&mut *core::ptr::addr_of_mut!(WORK)).try_reserve(1).is_err() {
        ...
        crate::service_sec_image::with_provider_process_manager(|pm| actor.release(pm))
            .expect("unentered WRITE actor release");
        source.release().expect("unentered WRITE allocation rollback");
    ...
    let token = match NEXT_TOKEN.fetch_update(Ordering::Relaxed, Ordering::Relaxed, |next| {
        ...
        Err(_) => {
            ...
            crate::service_sec_image::with_provider_process_manager(|pm| actor.release(pm))
                .expect("unentered WRITE actor release");
            source.release().expect("unentered WRITE token rollback");
            return Some(STATUS_INSUFFICIENT_RESOURCES_LOCAL);
        }
    };
    let work = Work {
        source, source_instance, provider_instance, caller, actor, route, dispatch, reply, token,
    ...
    if runtime::park_retained_service(route, token).is_err() {
        let mut work = (&mut *core::ptr::addr_of_mut!(WORK))[index]
            .take().expect("unparked WRITE owner");
        work.actor_release().expect("unparked WRITE actor");
        work.source.release().expect("unparked WRITE source");
        return Some(STATUS_INSUFFICIENT_RESOURCES_LOCAL);
    }
    None
}
...
impl Work {
    ...
    unsafe fn dispatch_provider(&mut self, handler: *mut ExecNtHandler) {
        ...
        let request = ExternalFileIrpRequest {
            ...
            parameters: IoParameters::Write(ReadWriteParameters {
                length: write.length(), key: write.key(), offset: write.byte_offset(),
            }),
            stack_flags: write.stack_flags(),
            initial_information: 0,
        };
        let result = hosted_file_owners::dispatch(
            self.caller, request, write.bytes(), &[], ExternalFileIrpDispatchPolicy::File,
        );
        ...
        let prepared = self.source.prepare().expect("entered WRITE source owner");
        let identity = prepared.identity();
        let invocation = prepared.begin(io_manager_mut(), identity)
            .expect("entered WRITE forward identity");
        let outcome = match result {
            hosted_file_owners::DispatchResult::Returned { status, information, .. } => {
                self.initial_status = Some(status.raw());
                WriteForwardOutcome::Returned(WriteCompletion {
                    status: status.raw() as u32, information,
                })
            }
            hosted_file_owners::DispatchResult::Outstanding { irp_id } => {
                ...
                WriteForwardOutcome::Pending
            }
        };
        match invocation.returned(outcome).finish(io_manager_mut(), identity) {
            WriteForwardResult::Terminal(terminal) => self.terminal = Some(terminal),
            WriteForwardResult::Retained(retained) => self.retained = Some(retained),
            WriteForwardResult::Rejected { error, retained } => {
    ...
    unsafe fn reconcile_rejected_completion(
        &mut self,
        error: nt_io_manager::retained_write_forward::WriteForwardError,
        retained: RetainedWriteForward,
    ) {
        use nt_io_manager::retained_write_forward::WriteForwardError;
        if matches!(error, WriteForwardError::PendingTerminal | WriteForwardError::ExcessInformation) {
            let identity = retained.identity();
            match retained.complete(io_manager_mut(), identity, WriteCompletion {
                status: STATUS_INVALID_DEVICE_REQUEST_LOCAL as u32, information: 0,
            }) {
                WriteForwardResult::Terminal(terminal) => {
                    self.initial_status = Some(STATUS_INVALID_DEVICE_REQUEST_LOCAL);
                    self.terminal = Some(terminal);
                    return;
                }
                WriteForwardResult::Retained(retained)
                | WriteForwardResult::Rejected { retained, .. } => self.retained = Some(retained),
            }
        } else {
            self.retained = Some(retained);
        }
        panic!("completed WRITE result has no reconcilable target identity");
    }
    ...
    unsafe fn poll_provider(&mut self) {
        ...
        assert_eq!(completion.major, major::IRP_MJ_WRITE);
        let retained = self.retained.take().expect("pending WRITE forward owner");
        let identity = retained.identity();
        match retained.complete(io_manager_mut(), identity, WriteCompletion {
            status: completion.status, information: completion.information,
        }) {
            WriteForwardResult::Terminal(terminal) => self.terminal = Some(terminal),
            WriteForwardResult::Retained(retained) => self.retained = Some(retained),
            WriteForwardResult::Rejected { error, retained } => {
    ...
    unsafe fn publish_source(&mut self) {
        let completion = self.terminal.as_ref().expect("WRITE provider terminal").completion();
        let irp = self.source.source_irp_exec().expect("live source WRITE IRP");
        write_unaligned((irp + WDM_X64_IRP_IO_STATUS_STATUS_OFFSET) as *mut u32, completion.status);
        write_unaligned(
            (irp + WDM_X64_IRP_IO_STATUS_INFORMATION_OFFSET) as *mut u64,
            completion.information,
        );
        self.source.arm_callback_free().expect("deferred source WRITE IRP free");
    }
    ...
    unsafe fn advance_stopped_source(&mut self) -> bool {
        ...
        ).expect("stopped WRITE Call retirement");
        true
    }
    ...
    unsafe fn advance(&mut self, handler: *mut ExecNtHandler) -> bool {
        ...
        if runtime::retained_service_cancelled(self.route, self.dispatch, self.reply, self.token) {
            if self.retained.is_none() && self.terminal.is_none() && self.canonical_irp.is_none() {
                ...
                runtime::acknowledge_retained_service_cancellation(
                    self.route, self.dispatch, self.reply, self.token,
                ).expect("unentered WRITE cancellation");
                return true;
            }
        ...
        let status = self.initial_status.expect("WRITE dispatch status");
...
pub(super) unsafe fn acknowledge(
    ...
    work.source.validate_source().expect("ACK WRITE IRP and File identity");
    ...
    runtime::retire_stopped_acknowledged_retained_service(
        work.route, work.dispatch, work.reply, work.token,
    ).expect("ACK original WRITE Reply retirement");
    runtime::park_retained_service(route, token)
        .expect("ACK WRITE Call admission after original retirement");
