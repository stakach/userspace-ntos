...
use super::*;
use nt_io_manager::{
    FileIoWaitKey, SynchronousFileRetryIdentity, SynchronousFileRetryOutcome,
...
fn report_failure(waiter: SynchronousFileWaiter, status: u32) {
    if FAILURES.fetch_add(1, Ordering::Relaxed) < 16 {
        ...
        match waiter.key() {
            ...
            FileIoWaitKey::LocalOverlay(file_object) => {
                print_str(b"overlay/");
                print_u64(file_object);
            }
        }
        print_str(b" tid=");
...
unsafe fn send(waiter: SynchronousFileWaiter) -> bool {
    if waiter.native_call_transport {
        ...
        client_reply_on(
            waiter.reply_cap,
            18,
            u64::from(waiter.service_number),
            waiter.reply_mrs[1],
            waiter.reply_mrs[2],
            waiter.reply_mrs[3],
        )
...
unsafe fn retire_reply(nt_handler: &mut ExecNtHandler, identity: SynchronousFileRetryIdentity) {
    let waiter = {
        let view = (&*core::ptr::addr_of!(SYNCHRONOUS_FILE_WAITERS))
            .retry_delivery(identity)
            .expect("File retry retirement lost its exact owner");
        assert!(matches!(
            view.phase,
...
pub(super) unsafe fn deliver_file(nt_handler: &mut ExecNtHandler, key: FileIoWaitKey) {
    let Some(identity) = (&*core::ptr::addr_of!(SYNCHRONOUS_FILE_WAITERS)).next_retry_for_file(key)
    else {
        return;
    };
    let phase = (&*core::ptr::addr_of!(SYNCHRONOUS_FILE_WAITERS))
        .retry_delivery(identity)
        .expect("selected File retry disappeared")
        .phase;
    if matches!(phase, SynchronousFileRetryPhase::Acknowledged { .. }) {
        retire_reply(nt_handler, identity);
        return;
    }
    let mut attempt = (&mut *core::ptr::addr_of_mut!(SYNCHRONOUS_FILE_WAITERS))
        .begin_retry(identity)
        .expect("selected File retry changed before entry");
    ...
    let result = if !wait_reply_pool_ref()
        .iter()
...
/// Retry acknowledged reply-cap retirement only. Failed/abandoned Reply is never replayed.
pub(super) unsafe fn redrive_retirement(nt_handler: &mut ExecNtHandler) {
    let mut previous_file = None;
    while let Some(identity) = (&*core::ptr::addr_of!(SYNCHRONOUS_FILE_WAITERS))
        .next_acknowledged_retry_after(previous_file)
    {
        previous_file = Some(
            (&*core::ptr::addr_of!(SYNCHRONOUS_FILE_WAITERS))
                .retry_delivery(identity)
                .expect("File reply retirement owner disappeared")
                .waiter
                .key(),
        );
        retire_reply(nt_handler, identity);
