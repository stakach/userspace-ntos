//! Source-domain ownership of a zero-payload hosted FLUSH_BUFFERS forward.
...
use nt_io_manager::{
    ...
    retained_flush_forward::{
        FlushCompletion, FlushForwardIdentity, PreparedFlushForward, TerminalFlushForward,
    },
...
pub(super) struct CapturedSourceFlush {
    ...
    pinned: bool,
    forward_identity: Option<FlushForwardIdentity>,
    target_retired: bool,
}
...
impl CapturedSourceFlush {
    ...
    pub(super) unsafe fn validate_packet(&self) -> Result<(), CaptureError> {
        self.validate_source()?;
        let inst = instance(self.instance_index).ok_or(CaptureError::InvalidSourceIrp)?;
        ...
        if irp.type_ != WDM_X64_IO_TYPE_IRP as i16
            ...
            || stack.major_function != major::IRP_MJ_FLUSH_BUFFERS
    ...
    pub(super) fn prepare(&mut self) -> Result<PreparedFlushForward, CaptureError> {
        ...
        let prepared = PreparedFlushForward::new(self.source, target, self.file_id());
...
pub(super) unsafe fn capture(
    ...
    target_device_address: u64,
) -> Result<CapturedSourceFlush, CaptureError> {
    ...
    let result = (|| {
        ...
        if initial_stack.major_function != major::IRP_MJ_FLUSH_BUFFERS
        ...
        let mut target =
            match HostedForwardTarget::capture(io_manager_mut(), domain, target_device_address) {
                ...
                Err(_) => {
                    source_file
                        .release()
                        .expect("unentered FLUSH File rollback");
                    return Err(CaptureError::InvalidTarget);
                }
            };
        let captured = (|| {
            ...
            Ok(StackFlags::from_bits_retain(initial_stack.flags))
        })();
        let stack_flags = match captured {
            Ok(flags) => flags,
            Err(error) => {
                target
                    .release(io_manager_mut())
                    .expect("unentered FLUSH target rollback");
                source_file
                    .release()
                    .expect("unentered FLUSH File rollback");
                return Err(error);
    ...
    if result.is_err() {
        assert!(
            ...
            "unentered FLUSH source rollback"
        );
    }
    result
}
