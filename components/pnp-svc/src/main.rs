//! `ntos-pnp-svc` — the PnP Manager ISOLATED over SURT (spec Milestone 12).
//!
...
unsafe fn build_component_vspace(sub: u64, comp: u64, req: u64, rep: u64, with_driver: bool) -> u64 {
    ...
    if vspace_assign_asid(pml4) != 0 {
        panic!("PnP service VSpace ASID assignment failed");
    }
    let pdpt = alloc_slot();
    let _ = untyped_retype(CAP_INIT_UNTYPED, OBJ_X86_PDPT, PAGING_BITS, 1, pdpt);
    let pd = alloc_slot();
...
unsafe extern "C" fn _start(bootinfo: *const BootInfo) -> ! {
    ...
    print_str(b"[ntos-pnp] PnpMmioInterruptTest.sys + PnP Manager, isolated over SURT\n");
    ...
    // Driver Host (client): loads the driver, drives the lifecycle, reports the verdict.
    spawn_component(
