//! `ntos-io-manager` — the NT I/O Manager as two ISOLATED seL4 components.
//!
...
unsafe fn build_component_vspace(sub: u64, comp: u64, req: u64, rep: u64) -> u64 {
    ...
    if vspace_assign_asid(pml4) != 0 {
        panic!("I/O-manager VSpace ASID assignment failed");
    }
    let pdpt = alloc_slot();
...
unsafe extern "C" fn _start(bootinfo: *const BootInfo) -> ! {
    ...
    print_str(b"[ntos-io] NT I/O Manager - isolated client/server over SURT\n");
    ...
    print_str(b"[ntos-io summary: ");
    print_u64(verdict);
    print_str(b" passed, ");
    print_u64(7u64.saturating_sub(verdict)); // 7 client-side checks
    print_str(b" failed]\n");
