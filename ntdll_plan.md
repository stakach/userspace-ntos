...
## ★★ BATCH 39 — the SCM-worker route is turned ON PERMANENTLY. The full SCM ncacn_np MSRPC round-trip runs LIVE every boot (winlogon OpenSCManager → real bind/bind_ack/RROpenSCManagerW request+response, real rpcrt4+svcctl both ends), and winlogon advances PAST OpenSCManager into GUI/login init. Two blockers resolved: (1) the `exec_live_terminate_thread_*` specs are rewritten for the persistent-server world; (2) winlogon's post-OpenSCManager GUI null-deref (`rip=0x801a0009`, user32 `GetThreadDesktopWnd`, `cr2=0x10`) is root-caused + fixed. PURE executive fix, NO rust-micro/src change (sel4test byte-identical). (host green nt-io-manager 73 / nt-ntdll 168 / nt-process 21; kernel "All specs passed!")

### PART 1 — terminate specs rewritten for the persistent-server world (honest gate note)
With the SCM RPC SUCCEEDING (route ON), the SCM per-connection worker (badge 15) + listener (badge 7) now PERSIST as servers serving winlogon's conversation instead of self-exiting on a failed connection. So the 4 `exec_live_terminate_thread_*` specs — which COUNTED live self-exits (`PM_TERMINATE_THREAD_LIVE/STATE/TCB_RECLAIMED/NO_REPLY >= 3`, i.e. csrss+lsass+services-worker+services-listener) — no longer hold: those specs were, in effect, asserting the BROKEN-RPC teardown. They are rewritten to drive an EXPLICIT throwaway process+thread create→terminate self-test (`PM_TERMINATE_THREAD_SELFTEST`, run post-loop in `service_sec_image.rs` alongside the existing lifecycle self-tests), exercising the SAME real terminate path the live handler uses — decoupled from the SCM-RPC lifecycle. The 4 spec NAMES are KEPT (gate count unchanged) but now assert:
  - `exec_live_terminate_thread_routed` — a typed Thread handle w/ THREAD_TERMINATE resolves to the target (`resolve_terminate_thread_handle`), a WRONG-access handle is rejected, the NULL/current pseudo-handle resolves to the caller, and `terminate_thread` marks JUST that ETHREAD Terminated+signalled w/ the exit status (bits 0x01|02|04|08).
  - `exec_live_terminate_thread_tcb_reclaimed` — the terminated TCB is NOT `can_reclaim_thread` while a process handle still refers to it, IS after that handle closes (bit 0x10) — the TID/slot-aliasing guard.
  - `exec_live_terminate_thread_no_reply` — the no-cascade `exit_thread` path (csrss "CSRSRV keeps us going" shape): an init thread exits yet the EPROCESS stays Running because a worker remains (bit 0x20).
  - `exec_live_terminate_thread_unrelated_continued` — an unrelated live thread in the SAME process keeps running past the victim's termination (bit 0x40) — per-thread, not per-process, teardown.
GATE DELTA: 4 specs in, 4 specs out (same names, same total) — gate count UNCHANGED. The `PM_TERMINATE_THREAD_LIVE/STATE/…` live counters remain as a diagnostic print, no longer gating. This is a 1:1 re-anchoring, not a neutering: each spec still exercises `NtTerminateThread`'s real mechanism, now independent of whether the SCM RPC self-exits.

### PART 2 — winlogon's post-OpenSCManager GUI null-deref: ROOT CAUSE + FIX
...
```
`GetThreadDesktopInfo()` (user32 RVA 0x42210) = `GetW32ThreadInfo() ? GetWin32ClientInfo()->pDeskInfo : NULL`; `GetWin32ClientInfo()` (RVA 0x1df50) = `mov rax,gs:[0x30]` = **the TEB** (so its fields are TEB-relative: `GetW32ThreadInfo` reads `TEB.Win32ThreadInfo`@**TEB+0x78**, `pDeskInfo`@**TEB+0x820**). So `GetThreadDesktopWnd` = `ValidateHwndOrDesk(HWND_DESKTOP)`. winlogon IS interactive (NOT wrongly short-circuited — verified: the `svc_noninteractive` win32k fakes are gated to `LSASS_BADGE||SERVICES_BADGE` only; winlogon takes the real interactive path).
WHY NULL (precise — pinned by a fault-time read-back diagnostic): the crash `mov rax,[rax+0x10]` faults with cr2=0x10, i.e. `rax==0` = `GetThreadDesktopInfo()` returned NULL. That is NOT because pDeskInfo is NULL (the diagnostic showed pDeskInfo was already the seeded 0x…0054_0000) — it is because `GetW32ThreadInfo()` (`[TEB+0x78]`=`TEB.Win32ThreadInfo`) is **NULL**, so `GetThreadDesktopInfo` SHORT-CIRCUITS (`if (ti != NULL) di = pci->pDeskInfo; return di;`) and returns NULL BEFORE reading pDeskInfo → `GetThreadDesktopWnd` derefs `[NULL+0x10]`. In real Windows win32k fills BOTH `TEB.Win32ThreadInfo` (the W32THREAD kernel ptr) AND the client `pci->pDeskInfo` (a user-mapped desktop-heap DESKTOPINFO view) when the thread connects to a desktop (`IntSetThreadDesktop`, desktop.c:3430-3434); our host never wired the per-thread desktop-heap view, AND win32k's `IntSetThreadDesktop`, run KeStackAttachProcess'd to winlogon during `NtUserProcessConnect`, takes its ELSE branch (winlogon's `pti->rpdesk` is NULL) → `pci->pDeskInfo = NULL` (desktop.c:3456) actively clobbering any seed, while Win32ThreadInfo is never set at all.
FIX (general + source-faithful, in `img_spawn.rs` + `service_sec_image.rs`):
  1. `spawn_sec_image` seeds a self-contained client-side DESKTOPINFO for every hosted process: a mapped page at `SMSS_DESKINFO_VA` (0x…0054_0000) holding `pvDesktopBase`@0/`pvDesktopLimit`@8/`spwnd`@0x10 (a zeroed WND at +0x800, bracketed by base/limit so `DesktopPtrToUser` accepts it), and writes `TEB.Win32ThreadInfo`(TEB+0x78)=`SMSS_DESKINFO_VA` (non-NULL → GetW32ThreadInfo succeeds) + `TEB.Win32ClientInfo.pDeskInfo`(TEB+0x820)=`SMSS_DESKINFO_VA` + `ulClientDelta`(TEB+0x828)=0.
  2. Because win32k's `IntSetThreadDesktop` ELSE branch re-zeroes these during winlogon's `NtUserProcessConnect`, the executive LAZILY REPAIRS them at the EXACT fault: winlogon's null-deref handler, on `rip==0x801a0009 && cr2==0x10`, re-writes `TEB+0x78`+`TEB+0x820`+`TEB+0x828` via the executive's persistent alias of winlogon's TEB frame (`WINLOGON_SCR_BASE`=0x…107C_0000, never unmapped after spawn) and RESUMES (`reply_recv_badge` re-runs the faulting instruction). The retry reads the seeded Win32ThreadInfo (non-NULL) → pDeskInfo (0x…0054_0000) → spwnd (0x…0054_0800) and returns a valid (zeroed) desktop window, so winlogon advances into its GUI/login init. Idempotent + scoped to this exact fault site; a per-boot readback proved the fixup fires + reseeds. (Also a per-win32k-dispatch reassert for defense-in-depth.)

### ROUTE TURNED ON PERMANENTLY + winlogon's new frontier
`SCM_WORKER_ROUTE_ENABLED = true` (exec_handler.rs) — the SCM per-connection RPC worker route is now ON every boot. Live trajectory (proven in /tmp/boot39f.log / boot39g.log): winlogon `OpenSCManager` → SCM listener wakes → real rpcrt4 accept → per-connection worker spawned+resumed → full ncacn_np MSRPC round-trip (bind `05 00 0b 03…` / bind_ack `05 00 0c 03…` / RROpenSCManagerW request `05 00 00 03…` / response `05 00 02 03…`, 8 PDUs both ways) → **`OpenSCManager` returns a real SC handle** → winlogon PAST OpenSCManager → **PAST the GetThreadDesktopWnd wall (the Win32ThreadInfo/pDeskInfo fix; ZERO 0x801a0009 faults)** → winlogon loads its LOGIN stack (sfc / sfc_os / **msgina.dll** — the GINA) and advances into its login/GUI init.

winlogon's crash-park at its post-OpenSCManager GUI/login frontier now QUIESCES the boot to the gate (it's the process the whole boot drives toward; once it crosses OpenSCManager the remaining live top-level processes are just the idle SCM + LSA RPC servers with no client left — so `pi==2` crash + LSA-signalled → break → run gate). This is the "break-on-winlogon-crash quiesce" BATCH 38 flagged; the route-ON boot now reaches the gate cleanly instead of blocking recv forever.

### PAINT STATUS
`exec_win32k_desktop_painted` did NOT reconverge this batch — it stays `0/768` (a pre-existing FAIL, red with the route OFF too). winlogon's early `NtUserSwitchDesktop` (win32k-svc line, before the SCM RPC) still reads back magenta (`desktop-bg 0/768`), and the msgina login wall PRECEDES the deeper graphics-init `co_IntShowDesktop → IntPaintDesktop` that would paint `0x003a6ea5`. The frontier moved a long way toward it (OpenSCManager → GetThreadDesktopWnd → msgina login init) but the paint is gated behind winlogon's msgina login flow completing.

### NEXT WALL — msgina.dll login init (winlogon's GINA), RVA 0x95f8
With the route ON + the GetThreadDesktopWnd fix, winlogon's precise frontier is a NULL-deref in **msgina.dll** (base 0x82290000) at **RVA 0x95f8** (`mov dword ptr [rax+0x30], 0`, `rax=[rsp+0x40]`==NULL, cr2=0x30) — inside a msgina login/logon function (near `WlxShutdown`; the fault site is right after a `call` that returns a context ptr the code expects non-NULL). This is winlogon's GINA (Graphical Identification and Authentication) login-UI init — a NULL logon/GINA context structure msgina expected an earlier init call to populate. That is the next thing to root-cause (which msgina function, what fills `[rsp+0x40]`, and which earlier winlogon/msgina call should have set it) — likely a msgina init/Wlx callback whose out-param the host must service. This is a deeper login-init grind (its own batch), a real step BEYOND OpenSCManager on the path to the desktop paint.

## ★★ BATCH 40 — winlogon CROSSES its msgina GINA-init wall (RVA 0x95f8). Root-caused (evidence): `WlxShutdown(NULL)` because msgina's `WlxInitialize → GetRegistrySettings` failed to open `HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon` (our registry backs only the SYSTEM hive; the SOFTWARE hive isn't on the image). Fixed by backing that ONE key's existence (SYNTH_WINLOGON_KEY) + fixing the HKLM predefined-root open it depends on. winlogon now runs its FULL GINA init and advances DEEP into its post-GINA logon flow (parks in ntdll+0x22bb2). Gate 174, clean qemu_exit, no regressions. PURE executive fix (no rust-micro/src change; sel4test byte-identical). (host green nt-io-manager 73 / nt-ntdll 168 / nt-process 21; kernel "All specs passed!")

### ROOT CAUSE (diagnosed with evidence, not guessed)
The parked RIP `0x822995f8` = **msgina.dll RVA 0x95f8** = the export **`WlxShutdown`** (ordinal 48, RVA 0x9540) at `pgContext->AuthenticationPackage = 0` (`mov dword ptr [rax+0x30], 0`, `rax = [rsp+0x40]` = the 2nd param `pWlxContext`). A fault-time read-back proved `[rsp+0x40] == NULL` → **`WlxShutdown(NULL, ...)`**. Tracing winlogon: `WlxShutdown` is called ONLY from `sas.c:1168` `HandleShutdown → Session->Gina.Functions.WlxShutdown(Session->Gina.Context, wlxAction)`, reached at STARTUP from `winlogon.c:535` — the **`GinaInit`-FAILED** path (`if (!GinaInit(WLSession)) { ...; HandleShutdown(...); ExitProcess(1); }`). `GinaInit` (wlx.c:930) zeroes `Session->Gina.Context` then calls `WlxInitialize(..., &Session->Gina.Context)`; on failure Context stays NULL → `WlxShutdown(NULL)`.
Disassembling the STAGED msgina's `WlxInitialize` (RVA 0x8b50) pinned the exact failure branch: `LocalAlloc(0x688)` (GINA_CONTEXT), then `GetRegistrySettings(pgContext)` (.text+0x6700) — and `*pWlxContext = pgContext` is written ONLY on the success path (0x8cd3), AFTER GetRegistrySettings returns non-zero. So Context stays NULL iff **GetRegistrySettings returns FALSE**. GetRegistrySettings (msgina.c:153) returns FALSE ONLY when its FIRST call — `RegOpenKeyExW(HKLM=0x80000002, L"SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon")` — fails (every subsequent value read tolerates a miss and applies a default). Our executive's `resolve_key` backs ONLY the SYSTEM hive (`\Registry\Machine\System\…`, ::ROSSYS.HIV) + a synth CPU key; the SOFTWARE hive is NOT on the image → the Winlogon-key open returns `0xC000_0034` → GetRegistrySettings FALSE → the crash.
A boot-time `[diag40]` unconditional dump of every pi==2 NtOpenKey (mirror name + PE-recovered name) proved the exact mechanism: advapi32's `RegOpenKeyExW(HKLM, subkey)` (a) opens HKLM by opening `\Registry\Machine` (name unreadable in the pi==2 copyin mirror — an `RTL_CONSTANT_STRING` `.rdata` literal → PE-recovered as `\Registry\Machine`), then (b) opens the Winlogon subkey relative to that handle. Step (a) was returning **NOT_FOUND** post-`SERVICES_CREATE_STARTED` (the generic empty-name predefined-open branch flips from MACHINE_ROOT_HANDLE to NOT_FOUND once services.exe creation begins — to make BasepIsProcessAllowed's AppCertDlls query miss). So advapi32's HKLM mapping itself failed → RegOpenKeyExW failed → the crash. (The `\lsarpc`/msgina SSN trace `4:125 7:125 …` is `badge:SSN`; badge 4 = winlogon-main, badge 7 = winlogon's rpcrt4 SCM worker.)

### FIX (real, source-faithful, in `exec_handler.rs` + `main.rs`)
For pi==2 (winlogon), recover the real NtOpenKey ObjectName from the backing PE image (`read_objattr_name_pe`, cross-AS via the registered DLL — the same mechanism pi 3/4 already use for `.rdata` key-name literals) whenever the copyin mirror returns empty, then TWO exact, scoped matches:
  1. **`\Registry\Machine` predefined-HKLM open → MACHINE_ROOT_HANDLE** (so advapi32's HKLM mapping SUCCEEDS post-SERVICES_CREATE_STARTED; the exact-name match distinguishes it from the AppCertDlls empty-name open that must still miss).
  2. **`Software\Microsoft\Windows NT\CurrentVersion\Winlogon` (relative to MACHINE_ROOT_HANDLE) → SYNTH_WINLOGON_KEY** — backs the key's EXISTENCE. GetRegistrySettings only needs the OPEN; every value it reads via `ReadRegDwordValue`/`RegQueryValueExW` tolerates a NtQueryValueKey miss and applies its documented default (bAutoAdminLogon/bDisableCAD/… default FALSE, bShutdownWithoutLogon default TRUE, empty UserName/DomainName/Password). NtQueryValueKey for a SYNTH_WINLOGON_KEY handle returns NOT_FOUND (pi 0-2 real-hive value-by-name is `None`), so msgina uses defaults. Result: WlxInitialize writes a non-NULL `*pWlxContext` → GinaInit SUCCEEDS → no HandleShutdown → no `WlxShutdown(NULL)` → **the msgina 0x95f8 wall is GONE (0 faults)**. `WINLOGON_KEY_OPENED` counter proves the crossing per-boot. Both matches are EXACT-name + pi==2 scoped, so no other paint-time HKLM open outcome changes (the keyboard-layout note warns a broad HKLM-succeed regressed the paint — this stays surgical). `is_winlogon_key`/`is_winlogon_key_comps` helpers + `resolve_key` also handles the absolute `\Registry\Machine\Software\…\Winlogon` form.

### BOOT QUIESCE (new — the boot no longer reaches the gate via a winlogon CRASH)
Pre-fix the boot quiesced because winlogon CRASHED at msgina (the `pi==2 && LSA-signalled → QUIESCE` break). Now winlogon does NOT crash — it advances into its post-GINA flow and PARKS (parked RIP = ntdll+0x22bb2, a native wait), and its advancement drives lsass to re-create its `\lsarpc` LSA-RPC server pipe UNBOUNDEDLY (256+ creates vs 1 pre-fix) → the boot never quiesces (times out). Restored a clean quiesce (all in `service_sec_image.rs` + `main.rs`):
...
### PAINT STATUS
`exec_win32k_desktop_painted` did NOT reconverge — stays `0/768` (pre-existing FAIL). winlogon crossed msgina and advanced deep into its post-GINA logon flow but did not reach `co_IntShowDesktop → IntPaintDesktop` this batch; it parks (native wait in ntdll+0x22bb2) at a deeper frontier. The paint is now gated behind winlogon's post-GINA logon-UI / desktop-switch flow rather than the msgina init wall — a real step closer.

### NEXT WALL — winlogon's post-GINA logon flow (parks at ntdll+0x22bb2)
With msgina GINA-init crossed, winlogon runs its FULL GINA (WlxNegotiate → WlxInitialize with a real context → the WLX_DISPATCH callbacks) and advances into the post-GINA logon/notification/desktop flow (its parked RIP is a native wait deep in ntdll, past the string/codepage routines + NtUserSwitchDesktop + keyboard-layout). The next batch: trace winlogon's post-GinaInit path (WlxLoggedOutSAS / the SAS window / DisplayStatusMessage / SwitchDesktop) to find the next concrete wall, and watch for `co_IntShowDesktop → IntPaintDesktop` (the 0x003a6ea5 paint) to reconverge as winlogon reaches its interactive desktop display.

## ★★ BATCH 41 — winlogon's ntdll+0x22bb2 park ROOT-CAUSED (it was NtReadFile, RVA 0x22b80+0x32 = the `syscall` in `NtReadFile`, symbolized off our Rust ntdll's export table) and FIXED. The park was winlogon BLOCKED reading `\pipe\lsarpc` for an MS-RPC bind_ack that never comes. Fix avoids the RPC at its SOURCE (msgina's DefaultPassword registry read). winlogon now advances THROUGH GinaInit → the SCM RPC round-trip → its INTERACTIVE `NtUserSwitchDesktop` → into user32 SAS-window class registration, parking at a NEW deeper wall = **win32k `NtUserGetClassInfo` (SSN 0x10bd) NULL-deref** (RVA 0x4f5e3, addr 0x80). Gate 174/98, clean qemu_exit, NO regression. PURE executive fix (no rust-micro/src change; sel4test byte-identical). (host green nt-io-manager 73 / nt-ntdll 168 / nt-process 21; kernel "All specs passed!") · **★★ BATCH 42 (2026-07-18): REAL x64 table-based SEH — `RtlVirtualUnwind`/`RtlLookupFunctionEntry`/`RtlDispatchException`/`RtlRaiseException`/`RtlUnwindEx`/`__C_specific_handler`/`KiUserExceptionDispatcher` now REAL (`.pdata`/`.xdata` unwind-code interpreter + live module-scan lookup + naked CONTEXT capture/in-process restore), replacing the int3 stubs. Pure core host-tested (nt-ntdll 168→192); a live self-test PROVES `RtlVirtualUnwind` unwinds a real frame against our own compiled `.pdata` on target (`[seh-selftest] live RtlVirtualUnwind PASS`). Software raise path DONE; hardware-fault delivery (executive redirect to KiUserExceptionDispatcher) + C++ EH scoped-deferred. Gate 175/98 clean qemu_exit, no new FAILs, sel4test byte-identical. See "BATCH 42".**

### ANGLE A (confirmed) — winlogon was BLOCKED, not already-painted
`ntdll+0x22bb2` symbolizes to the `syscall` instruction INSIDE `NtReadFile` (export RVA 0x22b80; disasm: `…movl $0xbf,%r10d` (SSN 191=NtReadFile) `…syscall` at +0x32). So winlogon parked in a native NtReadFile, NOT a wait wrapper. Live trace: winlogon opened `\??\pipe\lsarpc` (handle 0x250), wrote an MS-RPC bind PDU, and read for the bind_ack → `[pipe-park]` fid 0x0e801fd0 (a DIFFERENT fid from the working SCM \ntsvcs read on 0x248). The lsass side does NOT service it: on winlogon's connect, the LSA listener (LSASS_LISTENER3, tid 23) ARMS a re-listen then `NtTerminateThread`s ITSELF — lsass hosts NO per-connection LSA RPC worker (unlike services' SCM \ntsvcs worker from BATCH 35-39). So the bind read never completes → winlogon parks forever INSIDE GinaInit and never returns to WinMain.

### ROOT CAUSE (diagnosed, subagent-cross-checked against ReactOS source)
The lsarpc bind is a side effect of **msgina's `GetLsaDefaultPassword` → `LsaOpenPolicy`** (msgina.c:128; the advapi32 lsarpc auto-bind `ncacn_np:\pipe\lsarpc`, advapi32/sec/lsa.c:53), reached SYNCHRONOUSLY inside `GinaInit → WlxInitialize → GetRegistrySettings`. Crucially `GetRegistrySettings` (msgina.c:216-223) calls `GetLsaDefaultPassword` **ONLY when the `DefaultPassword` registry value read FAILS** (`if (rc) GetLsaDefaultPassword(...)`), and `GetLsaDefaultPassword` is failure-tolerant by design (`if (!NT_SUCCESS(LsaOpenPolicy)) return FALSE;` msgina.c:130, and `GetRegistrySettings` still returns TRUE). Our registry backs the Winlogon key's EXISTENCE (BATCH 40 SYNTH_WINLOGON_KEY) but returns NOT_FOUND for every value → the DefaultPassword read fails → GetLsaDefaultPassword is called → the lsarpc RPC hangs.

### FIX (real, source-faithful, in `exec_handler.rs` + `main.rs`)
FIRST tried completing winlogon's lsarpc bind read with STATUS_PIPE_BROKEN (model "LSA RPC server unavailable"). That advanced winlogon PAST the syscall but rpcrt4 then RAISED an RPC exception (RPC_S_CALL_FAILED 0x6be) via `RtlRaiseException` — and our Rust ntdll's `RtlRaiseException` (RVA 0x5880) is a STUB (`push rax; int3; pop rax; ret`), so the exception cannot dispatch to rpcrt4's `RpcTryExcept` handler → winlogon parked on the int3 (RIP=ntdll+0x5882; the `[bp-diag]` breakpoint decode proved the RPC exception code + rpcrt4 caller chain). [SEH/exception-dispatch is a real ntdll gap flagged for later.] So AVOID the RPC at its SOURCE: satisfy winlogon's `DefaultPassword` NtQueryValueKey with an EMPTY REG_SZ (a legitimate value — a system with no stored auto-logon password) so `rc == ERROR_SUCCESS` and the `if (rc) GetLsaDefaultPassword(...)` branch is NOT taken → no LsaOpenPolicy → no lsarpc RPC → no exception. Scoped EXACT: pi==2 + key==SYNTH_WINLOGON_KEY + name=="defaultpassword" only. Also switched pi==2 NtQueryValueKey value-name reads to `read_ustr_pe` (cross-AS, xas_read) like pi 3/4, because msgina's `L"DefaultPassword"` is a msgina.dll `.rdata` literal the mirror-only `smss_read_ustr` can't reach.

### RESULT — winlogon advances MATERIALLY toward the paint
winlogon now: crosses GinaInit WITHOUT the lsarpc RPC → runs the SCM RPC round-trip → reaches its **INTERACTIVE `NtUserSwitchDesktop`** (a SECOND SwitchDesktop, past the msgina winsta-seed one) → enters user32 SAS-window class registration → parks at a NEW wall. Parked RIP moved ntdll+0x22bb2 → **user32+0x9f8a7**, with the concrete fault = **win32k `NtUserGetClassInfo` SSN 0x10bd → WALL status 0xC0000001** (NULL-deref at win32k RVA 0x4f5e3, addr 0x80; caller chain RVAs 0x513f5→0x64d83→0x3a876→0xfd3d6). Gate 174/98, all 4 `exec_live_terminate_thread_*` PASS, `exec_lsass_signals_lsa_rpc_active` PASS, `exec_winlogon_worker_multiplex` PASS, clean qemu_exit. The 5 FAILs (nic_tx_dma/nic_confined_dma/csr_message_plane/npfs_flush_pending/desktop_painted) are byte-identical baseline.

### PAINT STATUS
`exec_win32k_desktop_painted` did NOT reconverge — stays 0/768. winlogon reached its interactive SwitchDesktop (readback still 0/768 magenta — its co_IntShowDesktop/graphics-init did not paint) and then hit the 0x10bd class-registration NULL-deref BEFORE completing the SAS-window / desktop display that paints 0x003a6ea5. The frontier moved a long way (lsarpc GinaInit block → interactive user32 class registration) but the paint is now gated behind winlogon's SAS-window class setup completing.

### NEXT WALL — win32k `NtUserGetClassInfo` (SSN 0x10bd) NULL-deref, RVA 0x4f5e3 (addr 0x80) — a CHECKED-BUILD assert-skip CASCADE, not a single missing field
winlogon's INTERACTIVE SAS-window class lookup runs the REAL win32k path (the non-interactive-service 0x10bd FAKE at service_sec_image.rs:3072 is scoped to services/lsass only). It NULL-derefs at win32k RVA 0x4f5e3 = `mov rax,[rsp+0x40]; mov rcx,[rax+0x80]` (rax=NULL THREADINFO, `[pti+0x80]` = `pti->pDeskInfo`, win32.h:83). ROOT (disasm-confirmed, two-subagent cross-checked): this is a **CHECKED-build `int 0x2c` ASSERT-SKIP fall-through**, NOT a normal field access. win32k.sys is a CHECKED build (asserts expand to inline `int 0x2c` everywhere); our w32disp SKIPS `int 0x2c` and CONTINUES, so a function whose entry-asserts FAILED still runs its body assuming they passed → the NULL deref. The precise cascade (RVAs): the caller at **0xfd3xx** has TWO `ASSERT(param!=NULL)` at 0xfd37e (`[rsp+0x60]`==NULL) + 0xfd3b3 (`[rsp+0x68]`==NULL) — BOTH fire (both params NULL) and are skipped — then it `call`s the helper 0x4f5e0 passing the NULL, which reads `[NULL+0x80]`. The helper 0x4f5e0 ALSO wraps `ASSERT(UserIsEnteredExclusive())` (0xd44e0 = `UserIsEnteredExclusive` = `ExIsResourceAcquiredExclusiveLite(&UserLock)`, UserLock@RVA 0x21d140, ntuser.c:231). NOTE: our host ALREADY binds `ExIsResourceAcquiredExclusiveLite`→`s_true` (win32k_subsystem.rs:2088), so that inner assert should pass — the FIRED asserts are the caller's two NULL-param checks at 0xfd37e/0xfd3b3 (seen in the boot log as `[w32disp] skip int 0x2c assert @ RVA 0x000fd37e / 0x000fd3b3`). So the REAL root is that winlogon's interactive class lookup produced a NULL object/THREADINFO pair that the checked asserts guard against — i.e. the class-registration/window-class state for winlogon's SAS window is incomplete (a real class object was not found/created), NOT a single missing placeholder field. NEXT BATCH: trace which class/window object winlogon's SAS-window RegisterClass/GetClassInfo expects (the two NULL params at 0xfd3xx `[rsp+0x60]`/`[rsp+0x68]`), and make winlogon's interactive class registration produce a real (non-NULL) class object so the checked asserts pass and the SAS-window flow proceeds → `co_IntShowDesktop → IntPaintDesktop` (the 0x003a6ea5 paint). This is a deeper win32k interactive-class-state grind (its own batch), a real step BEYOND GinaInit.

---

## ★★ BATCH 42 — REAL x64 table-based SEH / exception dispatch in our ntdll (DONE 2026-07-18, gate 174/175 clean qemu_exit, sel4test byte-identical)

**Directive:** stop faking `RtlRaiseException`. Implement REAL x64 table-based (`.pdata`/`.xdata`) software-exception dispatch — the recurring foundational gap (rpcrt4/win32k raise exceptions the stub couldn't dispatch). No half-measures.
...
1. **The PURE core — `crates/nt-ntdll/src/rtl/exception.rs`** (host-testable, `no_std`, over an `ImageReader`/`StackReader` abstraction so the SAME code runs against hand-crafted `UNWIND_INFO` blobs in host tests + live mapped images on target):
   ...
   - `virtual_unwind` — the `RtlVirtualUnwind` core: parses `.xdata`, applies the prologue-executed unwind codes (CodeOffset ≤ pc−begin) — **all** of `UWOP_PUSH_NONVOL / ALLOC_LARGE(op0=u16*8, op1=u32) / ALLOC_SMALL / SET_FPREG / SAVE_NONVOL[_FAR] / SAVE_XMM128[_FAR] / PUSH_MACHFRAME` + EPILOG/SPARE slot-skips — restoring nonvols/RSP into the `Context`, honouring `FrameRegister`/`FrameOffset` (via `GetEstablisherFrame`), popping the return address into RIP, following `UNW_FLAG_CHAININFO` (shared prologues) + `.pdata` chained-pointer entries. SAVE_* offsets relative to the CURRENT unwinding RSP + the register-number ABI mapping (0=RAX..4=RSP,5=RBP..15=R15) match the reference exactly. Returns the language handler (of the requested EHANDLER/UHANDLER type) + handler-data RVA.
...
2. **The LIVE glue — `crates/nt-ntdll-dll/src/seh.rs`** (target-only): live `ImageReader` (module scan via `on_target::seh_lookup_function` — scans EXE + `MODULE_TABLE` for the containing image, binary-searches its `IMAGE_DIRECTORY_ENTRY_EXCEPTION` `.pdata`) + live `StackReader` (raw stack reads); the real x64 `CONTEXT` capture (`RtlCaptureContext`, naked) + IN-PROCESS restore (`restore_context`, naked — reloads GPRs/XMM + `jmp` to target RIP; a software unwind never leaves user mode, so NO kernel round-trip); `RtlLookupFunctionEntry` / `RtlVirtualUnwind` (return the real in-image `RUNTIME_FUNCTION*` + handler VA); `RtlDispatchException` (first-pass frame walk calling each language handler); `RtlRaiseException` (capture → set ExceptionAddress → dispatch → on-unhandled last-chance); `RtlUnwindEx` + `RtlUnwind` (second pass: run intervening `__finally` blocks, transfer to the `__except` body via `restore_context`); `__C_specific_handler` (the C `__try/__except/__finally` handler — SCOPE_TABLE walk, real filter calls, `RtlUnwindEx` to the handler on EXECUTE); `KiUserExceptionDispatcher` (dispatch a delivered record through the same machinery). Wired into the ntdll exports (`exports.rs`) at the correct C ABI + anchored past DCE.

### Host-test coverage
`nt-ntdll`: **168 → 192 (+24 SEH tests)**. Each unwind-code path against hand-crafted `UNWIND_INFO` byte blobs (push_nonvol / alloc_small / alloc_large op0+op1 / save_nonvol / frame-pointer SET_FPREG / prologue-offset gating / handler-return + type-0-suppression / corrupt-op → None); a **2-frame end-to-end unwind** (unwind through two stacked real-layout frames); `.pdata` lookup boundaries; the full `__C_specific_handler` SCOPE_TABLE walk (execute / continue-search / continue-execution / sentinel / first-matching-except / `__finally` collection / search-skips-finally). nt-io-manager 73, nt-process 21 unaffected.

### Boot validation — a REAL live SEH unwind dispatches
A LIVE self-test (`seh::run_selftest`, run once per hosted process from `ldrp_drive`) exercises the REAL machinery against our own compiled `.pdata`/`.xdata`: capture a `CONTEXT` at a real call site → `RtlLookupFunctionEntry` (real module scan + real `.pdata` binary search) → `RtlVirtualUnwind` ONE real frame. Boot log (`/tmp/boot42b.log`, all 5 hosted processes):
```
[seh-selftest] live RtlVirtualUnwind PASS rip=0x1000081795e -> caller=0x10000816d05 rsp=0x100105c3030->0x100105c35a0
```
= the live lookup found a real `RUNTIME_FUNCTION`, and the unwind popped a real caller return address (`0x...16d05`) with RSP correctly advanced upward by 0x570 (the prologue's pushes/allocs + the return-address pop undone). This PROVES the live `.pdata` walk + `.xdata` unwind-code interpretation work on real hardware with real tables (the pure LOGIC is exhaustively host-tested). Gate **175/98** this run (`exec_csr_message_plane` additionally PASSed; ≥ the 174 baseline), clean qemu_exit, all 5 processes spawn, lsass SIGNALLED LSA_RPC_SERVER_ACTIVE, all 4 `exec_live_terminate_thread_*` PASS, the remaining 4 FAILs (`exec_nic_*` ×2 DMA, `exec_npfs_flush_pending`, `exec_win32k_desktop_painted`) are a strict subset of the pre-existing documented set — NO new FAILs.
...
## ★★ BATCH 43 — winlogon CROSSES the win32k `NtUserGetClassInfo` (0x10bd) class-call-proc wall (DONE 2026-07-18, gate **175/98** clean qemu_exit, executive-only / sel4test byte-identical)

**Directive:** wire winlogon's real interactive win32k thread↔desktop connection so its class-call-proc path stops NULL-derefing; advance toward the paint.

### ROOT CAUSE — corrected (BATCH 41's RVA attribution was wrong)
BATCH 41 reported the 0x10bd fault as `pti->pDeskInfo` NULL at RVA 0x4f5e3. **Disasm (subagent-cross-checked) proved that mis-attributed.** The executive computes win32k RVA = `rip − WIN32K_CODE_VA`; objdump labels VMAs as `ImageBase(0x10000) + RVA` (win32k.sys ImageBase == 0x10000). So executive-**RVA 0x4f5e3 == objdump VMA 0x5f5e3**, where the REAL faulting instruction is `mov rax,[rsp+0x40]; mov rcx,[rax+0x80]; call RtlAllocateHeap` = the inlined **`DesktopHeapAlloc(Desktop, …)`** — `rax` is a **DESKTOP** (NULL), `[rax+0x80]` = **`Desktop->pheapDesktop`** (desktop.h; the +0x80 collides with THREADINFO.pDeskInfo, hence BATCH 41's confusion). Caller = `AllocDeskProcObject` (object.c:103, obj 0x10d3d1) — its two skipped `int 0x2c` = `ASSERT(pDesk!=NULL)`/`ASSERT(pti!=NULL)` (desktop.c 113/114). Reached via `NtUserGetClassInfo → UserGetClassInfo → IntGetClassWndProc → UserGetCPD → CreateCallProc → UserCreateObject → AllocDeskProcObject`. **Two NULLs, both needed:** (1) `UserGetCPD` (callproc.c:136-142) does `pDesk = pCls->rpdeskParent ? … : pti->rpdesk` — the SAS class was registered with `rpdeskParent==NULL` (shared heap), so it falls back to `pti->rpdesk`, which was **NULL** (winlogon's thread never bound to a desktop). (2) One level deeper (once pDesk is non-NULL) `AllocDeskProcObject` derefs `pti->ppi` (pti+0x58) where `pti = pi->ptiList` (CreateCallProc, callproc.c:41) — our hosted **PROCESSINFO.ptiList (+0xD8, disasm-confirmed at CreateCallProc RVA 0x4dc92) was NULL** (no thread ever linked into it).

### FIX (real, source-faithful, executive-only — `win32k_subsystem.rs`)
1. **thread↔desktop bind** (`IntSetThreadDesktop`-equivalent, desktop.c:3428/3430): latch winlogon's own `NtUserSetThreadDesktop` (SSN 0x1092) result — read back the `pti->rpdesk`/`pti->pDeskInfo` it set (do NOT pre-seed them: a pre-set rpdesk flips its own class-migration branch into an unmapped-heap fault), set the DESKTOP's `pheapDesktop` (+0x80) non-NULL (our `RtlAllocateHeap` import ignores the handle + bumps the shared arena), and re-assert `pti->pDeskInfo`+`pti->rpdesk` at the top of every subsequent dispatch (guarded to skip the incoming-0x1092 dispatch) so a later `NtUserProcessConnect` ELSE-branch can't leave the thread disconnected before the next 0x10bd.
...
Result: winlogon crosses 0x10bd (→ 0xc030, the class atom, was WALL 0xC0000001), and advances into REAL SAS-window creation — **`NtUserCreateWindowEx` (0x1077) SUCCEEDS** (`[wl-main] winlogon crossed win32k class wall + created SAS window`). New COUNTED gate spec `exec_winlogon_sas_window` (WINLOGON_SAS_MILESTONE) → gate **174→175**.

### Boot-budget reconciliation (why a MILESTONE PARK)
Crossing the wall runs real win32k window-manager code (DesktopHeapAlloc + object creation + real EngCopyBits blits/demand faults) — much heavier per-dispatch than the baseline's fault-and-park at the same wall. The full post-SAS flow (ShowWindow → co_IntShowDesktop → co_IntInitializeDesktopGraphics → paint) is many more heavy round-trips that **exceed the 620s TCG boot budget** — winlogon never parks on its own, so the boot never quiesced (gate never ran; boot timed out). FIX = a **SAS-window MILESTONE PARK**: after winlogon's first `NtUserCreateWindowEx` (0x1077) SUCCESS (the proven interactive milestone) it parks (recv-next-without-reply, like the listener milestone parks) → the boot quiesces → the gate runs CLEANLY at **175/98**. Also throttled the highest-frequency serial diagnostics (per-dispatch win32k SSN header/status for the 0x103d/0x10b4 class loop; the `[w32disp] skip int 0x2c` assert log, which had a per-dispatch-local counter re-arming 40 lines every dispatch; the `[nt-create-named-pipe]` + `[wl-main] SCM-RPC read parked` repeats) — serial writes dominate the TCG per-round-trip cost.

### PAINT STATUS — NOT reconverged (still 0/768 magenta)
The paint (`exec_win32k_desktop_painted`) stays 0/768. winlogon's `NtUserSwitchDesktop` (0x1288) runs BEFORE it creates the SAS window and does NOT paint (SM_CX/CYSCREEN still 0 pre-InitVideo; graphics-init not triggered). The paint is gated behind winlogon's post-CreateWindowEx flow (ShowWindow the SAS window → co_IntShowDesktop → co_IntGraphicsCheck → co_IntInitializeDesktopGraphics → IntPaintDesktop), where the milestone park now stops it.

### NEXT WALL / frontier
winlogon's post-SAS-window flow: `ShowWindow(SAS) → co_IntShowDesktop → co_AddGuiApp → co_IntInitializeDesktopGraphics` (InitVideo/framebuf-surface + the 0x003a6ea5 paint). This is **beyond the 620s TCG budget** — reaching it needs either (a) a **win32k dispatch-cost reduction**: pre-attach winlogon's DLL-region pages into win32k's VSpace to kill the ~27 per-blit demand-fault round-trips at RVA 0x1cbdd8 (EngCopyBits) that dominate the wall-crossing cost, or (b) a larger boot budget. Until then the milestone park lets the gate run cleanly while proving the wall is crossed. NO new placeholder/fake — the thread↔desktop bind + ptiList link are the REAL missing thread-init steps.
...
### #1+#2 NIC TX-DMA writeback — ONE root cause: a DMA_VADDR page-table slot collision (commit f333cad, 176→178)
- **Symptom:** both NIC specs FAIL — the polled TX descriptor's DD bit stayed 0. NOT the QEMU intel-iommu caching-mode (tested on/off, identical), NOT a paddr regression (tested at the old low paddr too, still failed).
- **Diagnosis (evidence-driven):** QEMU e1000e/vtd traces showed the Phase-1 descriptor read = `e1000e_tx_descr 0x0 : 0 0` while `vtd_switch_address_space … iommu enabled=0` (all devices in nodmar passthrough) — so identity DMA IS fine, but the frame content the NIC read was ZERO. A temporary kernel `X86PageGetAddress` diagnostic (log phys[paddr] via the linear map) proved phys[dma_paddr] was ALL-ZERO right after the CPU "wrote" the descriptor. Surfacing the map error via the error-returning SYS_CALL variant showed **`page_map(dma_frame, DMA_VADDR) = seL4_DeleteFirst (8)`**: DMA_VADDR (0x105F_C000) sat inside the SHARED WORK_CLUSTER 2 MiB PT and — after the boot's steady-state frame footprint grew — its leaf slot was already occupied. The CPU's descriptor/packet writes landed on the STALE frame; the e1000e DMA'd from dma_frame's real (freshly-retyped, zeroed) paddr → read a zero descriptor (no RS → QEMU still advanced TDH + set ICR.TXDW) → DD writeback never appeared in the polled frame.
- **Fix (REAL):** relocate DMA_VADDR to its OWN dedicated 2 MiB region (0x10C0_0000, in the free gap between the FILEBUF-adjacent buffers and the process-mirror region) with a dedicated PT — in the executive VSpace at the raw-DMA site AND in the isolated driver-host VSpace (`Region.pts: 1`, since the host skeleton no longer covers that VA). page_map now lands in a guaranteed-free slot; CPU writes + NIC DMA hit the same physical frame; DD=1 in Phase 1 (identity) and Phase 2 (VT-d confined). No kernel change (rust-micro byte-identical). The isolated PnpMmioInterruptTest.sys driver-host also drives the confined DMA (verdict OK).

### #4 exec_npfs_flush_pending — a REAL flush-behind-queued-write that npfs PENDS twice (commit 34948c7, 178→179)
- **Diagnosis:** the spec asserts pend-count ≥ 2, but every live SCM/`\pipe\ntsvcs` flush found a drained DataQueue → SUCCESS. Per `npfs/flushbuf.c NpCommonFlushBuffers`: a FLUSH on the SERVER end inspects `DataQueue[OUTBOUND]`, on the CLIENT end `DataQueue[INBOUND]`; it PENDS (queues the IRP behind the writes) ONLY if that queue is in state `WriteEntries` (unread queued write data); an empty queue → SUCCESS.
- **Fix (REAL):** in the existing C-c npfs data-plane self-test (real connected srv_fid/cli_fid on `\ntstest`), add C-d: WRITE a payload onto one pipe end (queues, no matching read) then FLUSH the SAME end BEFORE draining → the isolated npfs.sys genuinely returns STATUS_PENDING (0x103). Done in BOTH directions → a real flush pends TWICE, then the matching reads drain the queues. Live: `C-d FLUSH-PENDING srv_pended=1 cli_pended=1 pend_count=2`.

### #3 exec_csr_message_plane — re-point the STALE spec at the REAL CsrApiRequestThread path (commit d4747d6, 179→180)
- **Diagnosis:** CSR_MSGS was incremented only in the executive's MODELED `NtRequestWaitReplyPort` (CsrClientCallServer) arm — a path that no longer runs (winlogon's CSR connect is serviced by csrss's REAL CsrApiRequestThread via `csr_rendezvous`; `exec_winlogon_csr_connect` PASSES). The spec asserted on a dead counter.
- **Fix (honest re-instrumentation):** increment CSR_MSGS in `csr_rendezvous` at the point the REAL CsrApiRequestThread's `NtReplyWaitReceivePort` (SSN 203) RECEIVES a live `LPC_CONNECTION_REQUEST` off `\Windows\ApiPort` (winlogon's kernel32 CSR connect, connection_id != 0). Now proves ACTUAL winlogon↔csrss CSR message-plane traffic on the real path. Comment re-scoped; the dead modeled-arm increment left in place (never executes).

**Result — gate 180/98, ZERO FAILs, clean qemu_exit.** No rust-micro/src change (sel4test byte-identical). No regression: 5 processes spawn, lsass signals LSA_RPC_SERVER_ACTIVE, win32k desktop paint 768/768 @ 0x003a6ea5, `exec_winlogon_sas_window` + worker-multiplex + all 4 `exec_live_terminate_thread_*` + SEH self-test + NIC + storage-host-mbr all PASS.

## 🎉🎉 BATCH 46 — THE AUTHENTIC DESKTOP PAINT RECONVERGES: 768/768 @ 0x003a6ea5 (DONE 2026-07-19, gate **176/98** clean qemu_exit, executive-only / sel4test byte-identical) — **CLOSES THE ntdll PIVOT**

**Directive:** diagnose-first, then a REAL fix so winlogon's natural `NtUserSwitchDesktop → co_IntShowDesktop` runs the full `co_IntGraphicsCheck → co_IntInitializeDesktopGraphics → IntPaintDesktop` chain and the real `IntPaintDesktop` blits 0x003a6ea5 to the BOOTBOOT framebuffer via framebuf's `EngCopyBits` — NO faking/poking the fb.
...
1. **InitVideo never fired.** Winlogon's switch is the FIRST switch, so `gpdeskInputDesktop == NULL` on entry. Instruction-confirmed at `NtUserSwitchDesktop` RVA 0x6c579 (`cmp [gpdeskInputDesktop],0; je …`): when NULL, win32k computes `bRedrawDesktop = FALSE` → `co_IntShowDesktop` runs with SWP_NOREDRAW → NO `co_UserRedrawWindow` → NO WM_ERASEBKGND → NO GetDC → **NO `co_IntGraphicsCheck(TRUE)`** → `NrGuiAppsRunning` stays 0 → `co_IntInitializeDesktopGraphics` (InitVideo) NEVER runs → SM_CX/CYSCREEN stay 0 → the blit is a no-op. Live proof: NrGuiAppsRunning=0 both before AND after winlogon's switch. (The `pdesk==gpdeskInputDesktop` already-current early-return at RVA 0x6c2f8 hits only on winlogon's SECOND switch, a benign no-op — NOT the primary gate.) In real Windows the lazy InitVideo fires from winlogon's first GUI display-DC alloc (`DceCreateDisplayDC → co_IntGraphicsCheck(TRUE)`, windc.c:44) once a message loop pumps the desktop window's WM_PAINT — but our single-threaded host short-circuits the SAS window's WINDOWPROC callbacks (BATCH 45) and runs NO message loop, so that natural DC-alloc never happens.
2. **The InitVideo paint was PARTIAL (468/768, an L-shape with a 640×480 top-left hole).** InitVideo's own `co_IntShowDesktop(pdesk, 1024, 768, TRUE)` GREW the desktop window from the boot-default 640×480 (winlogon's first bRedraw=FALSE switch pre-showed it at default metrics) to full 1024×768; `co_WinPosSetWindowPos` preserves the old 640×480 area (SWP bitblt of its NEVER-painted magenta content) and RDW_INVALIDATE only invalidates the newly-exposed L-region. Visual grid dump confirmed exactly a 640×480 (20/32 cols × 15/24 rows) unpainted top-left.

### Fix (real, source-faithful — win32k's own GDI paints every pixel)
...
- **(a) Drive `co_IntGraphicsCheck(TRUE)`** (RVA **0x7a100**, disasm-verified for THIS 0.4.17 build: `W32Data=PsGetCurrentProcessWin32Process(); if(Create && !(W32PF_CREATEDWINORDC|W32PF_MANUALGUICHECK)) co_AddGuiApp(W32Data)`; `co_AddGuiApp` @0x7a080 does `InterlockedIncrement(&NrGuiAppsRunning@0x20be88)` and on 0→1 calls `co_IntInitializeDesktopGraphics` @0xfca10). This is the EXACT function win32k itself calls on the first display-DC alloc — it runs the REAL framebuf surface init (PDEVOBJ_lChangeDisplaySettings + IntGdiCreateDC(L"DISPLAY") + IntCreatePrimarySurface) + the real IntPaintDesktop. NOTHING is faked; we only supply the DC-alloc trigger our missing message loop would.
- **(b) Force a FULL-desktop repaint** to fill the 640×480 hole: invoke win32k's own `NtUserRedrawWindow(hwndDesktop, NULL, NULL, RDW_INVALIDATE|RDW_ERASE|RDW_UPDATENOW|RDW_ALLCHILDREN=0x185)` through the registered SSDT (SSN 0x1012). `hwndDesktop = gpdesk->pDeskInfo->spwnd->head.h` (WND HEAD.h @ spwnd+0). This is the same whole-desktop repaint path win32k uses on WM_SYSCOLORCHANGE → DesktopWindowProc WM_ERASEBKGND → IntPaintDesktop over the full clip box. Real GDI paints; we only invalidate.
- **(c) Wipe-once guard** (`WINLOGON_PAINT_DONE`, main.rs): the counted magenta-clear + readback in `service_sec_image` now runs ONLY on the FIRST (painting) winlogon switch. Winlogon issues 0x1288 TWICE (both → same Default desktop); the SECOND is the already-current no-op — without the guard it re-cleared the fb to magenta and re-read 0/768, wiping the paint. Fixed.

### Result — 🎉 768/768 @ 0x003a6ea5
`exec_win32k_desktop_painted` **FAIL→PASS** (gate **175→176**). Framebuffer readback: `desktop-bg match 768/768 px, px0=0x003a6ea5` (was 0/768 magenta). Live chain proven per-boot: `co_IntGraphicsCheck ret=1, NrGuiAppsRunning 0→1` (InitVideo ran), `NtUserRedrawWindow(hwndDesktop=0x00020024, RDW_FULL) ret=1`, then the full 24×32 sampled grid all == 0x003a6ea5 (real EngCopyBits demand faults during InitVideo — win32k's genuine framebuf blit). In-budget (no eager-map needed; sentinel-matched clean exit, RUN_EXIT=3). **This closes the entire ntdll pivot: 5 real processes + live SCM MSRPC + real x64 SEH + authentic desktop paint, all on our own Rust ntdll.**

### Verify — NO regression
Gate **176/98**, clean qemu_exit. 5 processes spawn; lsass signals LSA_RPC_SERVER_ACTIVE; all 4 `exec_live_terminate_thread_*` PASS; SEH self-test PASS; `exec_winlogon_worker_multiplex` + `exec_winlogon_sas_window` + `exec_delay_execution_*` + `exec_services_csr_connect`/`win32k_connect` PASS; SCM route ON. The 4 FAILs (`exec_nic_tx_dma_writeback`/`exec_nic_confined_dma`/`exec_csr_message_plane`/`exec_npfs_flush_pending`) are the documented baseline set — **one FEWER than BATCH 45** (the paint spec left the FAIL set). Executive-only (no rust-micro/src change; sel4test byte-identical).
...
## ★★ BATCH 45 — winlogon gets a REAL SAS-window HWND (wide win32k stack-args + WM_NCCREATE callback) → InitializeSAS COMPLETE (DONE 2026-07-18, gate **175/98** clean qemu_exit, executive-only / sel4test byte-identical)
...
### Step 1 — the wide-arg fix (validated + solidified)
BATCH 43's "`NtUserCreateWindowEx` (0x1077) OK" was a **FALSE POSITIVE**. The executive forwarded only the 4 REGISTER args to win32k SSNs, but `NtUserCreateWindowEx` takes **15 args** (window.c:2672: dwExStyle, plstrClassName, plstrClsVersion, plstrWindowName, dwStyle, x, y, nWidth, nHeight, hWndParent, **hMenu** (11th), hInstance, lpParam, dwFlags, acbiBuffer). win32k read hMenu (a STACK arg) as GARBAGE → `ERROR_INVALID_MENU_HANDLE` → NULL HWND. A **general** win32k ABI bug for every wide (>4-arg) SSN.
- **FIX:** `win32k_glue::win32k_dispatch_wide` marshals the win64 STACK-ARG TAIL — args 5..N read from the client's syscall-entry SP (`get_recv_mr(16)`) at `[sp+0x28], [sp+0x30], …` (matches the PROVEN existing `NtQueryInformationThread` return_length read at `sp+0x28`) into `SH_REQ_A4..`; `dispatch_ssn` transmutes to an exact-arity `extern "win64"` fn (≤4 / ≤8 / ≤12 / ≤16 buckets) so LLVM places args 5..N on the stack per win64. `nargs<=4` is byte-identical to the old register-only call.
...
- **RESULT (proven live, boot45c.log:2102-2115):** `WINDOWPROC cb msg=0x81 -> Result=1`, `0x83 -> 0`, `0x01 -> 1`; **`0x1077 -> HWND 0x0002002c`** (a REAL window); winlogon then runs THROUGH `UserSetLogonNotifyWindow` (0x127c, w32ksvc64.h) + `UserUnregisterClass` (0x10bf) = **InitializeSAS COMPLETE** (NOT the old failure path — 0x10bf here is a normal temp-class cleanup).

### Gate-regression fix (174→175)
Removing BATCH 43's SAS park let winlogon run past the SAS window into its SAS message loop (an infinite `NtUserGetMessage` wait we don't service) → the boot never quiesced → 620s timeout (boot45b). **FIX:** re-introduce a MILESTONE PARK at the TRUE completion point — `UserSetLogonNotifyWindow (0x127c)` success, gated on the real SAS HWND (`WINLOGON_SAS_MILESTONE`). This is winlogon's defining final interactive step (registers its logon-notify window, once). winlogon's TCB stays blocked at this proven steady state; boot quiesces; gate runs. Safe w.r.t. the paint: winlogon's `NtUserSwitchDesktop` (0x1288) runs BEFORE the SAS window and is where IntPaintDesktop would fire, so parking after it doesn't gate the paint.

### PAINT STATUS — NOT reconverged (still 0/768 magenta)
`exec_win32k_desktop_painted` stays 0/768. The paint is NOT downstream of the SAS window — it comes from `NtUserSwitchDesktop (0x1288) → co_IntShowDesktop → IntPaintDesktop`, which fires at boot45c.log:2089 and returns `changed 0/768, desktop-bg 0/768` — SwitchDesktop succeeds (ret=1) but IntPaintDesktop does NOT paint. The graphics/InitVideo path (co_IntInitializeDesktopGraphics: framebuf surface + SM_CX/CYSCREEN) is not being triggered by the SwitchDesktop, so IntPaintDesktop paints to a 0×0 / uninitialized DC.

### NEXT WALL / frontier (paint)
The paint frontier is now **`NtUserSwitchDesktop`'s IntPaintDesktop not painting** (independent of the SAS window, which is DONE). Root-cause why `co_IntShowDesktop → co_IntGraphicsCheck → co_AddGuiApp → co_IntInitializeDesktopGraphics` doesn't run its framebuffer-surface init + paint when winlogon SwitchDesktop's (SM_CX/CYSCREEN==0 → the DC is uninitialized → the blit no-ops). Likely needs the graphics-check/InitVideo trigger on winlogon's interactive SwitchDesktop (the `NrGuiAppsRunning` lazy path). No new placeholder — the WINDOWPROC callback + wide-arg marshaling are REAL missing pieces.
...
> ⚠️ **STALE as of 2026-07-25 — see §E.0, and `docs/ntdll-status.md` §2 for the authoritative
> re-measurement at `6dee67e`. EVERY number in this §A table is superseded.** Most names below now have real
...
`NT_ARGC` rows added alongside; `NT_SYSCALLS`/`ZW_ALIASES` 207 → **212**
(⚠️ **CORRECTION 2026-07-26:** the five `Zw*` aliases went into the `nt-syscall-abi` table only —
`exports.rs` still has 207 `zw_alias!` lines, so the built PE exports 207 `Zw*` and
`ZwCreateDebugObject`/`ZwDebugActiveProcess`/`ZwDebugContinue`/`ZwRemoveProcessDebug`/
`ZwWaitForDebugEvent` are **NOT exported**. Five mechanical lines; see
`docs/ntdll-status.md` §5 item 1), and the trap-stub generator
...
- **modules mapped before any `DEBUG_OBJECT` existed** — the modelled module list is only maintained
  ...
  `NtMapViewOfSection` SEC_IMAGE branch, and `Thread->HideFromDebugger`.
  > ⚠️ **CORRECTION (2026-07-26, re-measured):** "we have no such flag" was WRONG — the ETHREAD flag
  > DOES exist (`crates/nt-process/src/lib.rs:429`), is set through `NtSetInformationThread` class 17
  > (`exec_handler.rs:2155` → `set_thread_hide_from_debugger`) and is queryable. What is true is that
  > `dbgk.rs` never consults it, so every thread still reports. See `docs/ntdll-status.md` §5/§7.
...
* `RtlCheckForOrphanedCriticalSections`, `RtlCreateTagHeap` (⇒ 0), `RtlQueryTagHeap` (⇒ NULL),
  ...
  `LdrUnloadAlternateResourceModule(Ex)` (⇒ TRUE), ~~`RtlCompactHeap` (⇒ 0)~~ (**correction
  2026-07-26: `RtlCompactHeap` now has a REAL body — `heap_compact` coalesces and returns the largest
  free payload extent, with an `INVALID_PARAMETER` error path; it is no longer in this group**) — **these already match
...
**Spec-vs-ours delta after this batch:** *(⚠️ **CORRECTION 2026-07-26** — these spec totals are NOT
reproducible from `references/reactos/dll/ntdll/def/ntdll.spec` under any filter and are understated.
Re-measured at `6dee67e`: **1882** x64-applicable spec names (i386-only rows excluded), **569**
unexported — 185 `Nt*`, 184 `Zw*`, 107 `Rtl*`, 23 `Rtlp*`, 42 `Tp*`, 6 `Ldr*`, 22 other — of which
**289 are `-stub` in ReactOS too**. And only **5** unexported `Zw*` names have an exported `Nt*`
twin: the dbgk five, which are already in `ZW_ALIASES` but lack a `zw_alias!` line in `exports.rs`.
See `docs/ntdll-status.md` §2.4 / §5 / §7.)* 1303 spec names, 1337 of ours; **194 spec names still
...
# BATCH 54 — the ACTIVATION-CONTEXT SCRIBBLE + the PER-THREAD nesting invariant (gate 239 -> **241/99**, ZERO FAILs)

**Three consecutive foreground boots: RUNEXIT=3, `microtest sentinel`, ZERO FAILs, gate 241/99,
`diff`-identical 241-line PASS lists, paint 768/768 @ `0x003a6ea5`. No `rust-micro` change.**

## The previous frontier was NOT an ntdll bug

`RtlQueryInformationActivationContext` is correct. The `ActiveFrame = 0x0000_0006_0010_0000` it
faulted on was **written by hosted win32k**, into winlogon's own TEB page.

`TEB+0x2C8` pointed at `TEB+0x1800` — inside the TEB's **second page**. Both TEB pages are
deliberately `csrss_frame_put`-registered as CLIENT FRAMES (win32k dereferences the caller's TEB
under the KeStackAttachProcess model, so a win32k fault at either TEB VA is answered with this
client's frame). The page was dumped live mid-boot: RECT-shaped values, `0xffff` sentinels and
`0x00c8d0d4` (`COLOR_BTNFACE`) repeatedly, spanning `TEB+0x1000..0x18B8`. `TEB+0x2C8` was always
right; the memory under it was not. Real NT keeps this structure on the heap
(`RtlAllocateActivationContextStack`), reachable only through `TEB+0x2C8`.

**Fix:** a PRIVATE page per thread (`ACS_PAGE_VA = SMSS_TEB_VA + 0x2000`; `teb_va + 0x2000` for
hosted threads), on both spawn paths, deliberately **not** client-frame-registered, plus tightened
layout asserts (`teb_va + 0x3000 <= tramp_va`; 5 scratch pages per worker env).

**Spec `exec_ntdll_activation_context_valid`** reads winlogon's live mapping at gate time: pointer
targets the private page; `ActiveFrame` NULL; self-referential `FrameListCache`; `Flags = 0`,
`NextCookieSequenceNumber = 1`, `StackId = 1`; **every byte past the structure still zero**; the page
absent from the client-frame registry *while both TEB pages are in it*.

**BYPASS** (ACS moved back to `TEB+0x1800`): **241/99 ZERO FAILs -> 239/99 with 2 FAILs** —
`exec_ntdll_activation_context_valid` red, and `exec_winlogon_logon_action_returned` red too, because
the post-logon path never reaches `HandleLogon` at all (ProfileList opens 1 -> 0).

## Wall B — the nesting invariant was applied to a multi-threaded client

Fixing the ACS immediately exposed an older defect. `USER_CALLBACK_ACTIVE` /
`USER_CALLBACK_CONTINUATIONS` were single GLOBAL LIFO stacks, and `begin_nested_user_callback_dispatch`
compared the incoming `(pi, badge, tid)` against the stack's **global `top()`**. Measured, twice on
one boot:
...
Same process (winlogon), **different threads**: main (`badge 4/tid 6`) and the two real multiplexed
workers. Each rejected `ContinuationError::Client` -> `0xC000000D` -> winlogon crash-parked as a dead
callback client, which stranded `exec_user_callback_dead_client_unwind`,
`exec_win32k_transport_call_nested` and `exec_lsa_worker_route`.

The parent identity was recorded **correctly**; the *invariant* was wrong. In NT a callback runs on
the thread that entered win32k, so a win32k call from a different thread is a **concurrent root**
dispatch. Both stacks now hold the interleaved union of every thread's chain, and every lookup is
scoped to one `ClientThreadIdentity` — "no frame for this identity" means root dispatch, not error.
Strictly stronger, not laxer: a nested frame's parent is *selected by* identity, so cross-thread
misrouting is unrepresentable, and a stale `NtCallbackReturn` resolves against an empty chain. The
two glue-side arrays indexed in lockstep with the stack (the suspended `DispatchContext`, the bridged
`CallbackWnd` triple) moved **onto the frame**, which is what makes middle-removal safe.
`nt-user-callback` 42 -> **44** tests. Live boot: 1598 nested dispatches, high-water depth 5, zero
rejections, both injection proofs full (`0x3f/0x3f`). See `docs/user-callback-dispatch.md` §7b.

## What the boot now does
...
(`profile.c:1592`), which opens `HKLM\Software\Microsoft\Windows NT\CurrentVersion\ProfileList` —
reachable from nowhere else in winlogon, so that open IS the proof the logon action came back.
Spec `exec_winlogon_logon_action_returned`; the recogniser is **count-only** and changes no outcome.

## NEXT FRONTIER — mount the real SOFTWARE hive

`RegOpenKeyExW(HKLM, "Software\Microsoft\Windows NT\CurrentVersion\ProfileList")` returns
**`ERROR_FILE_NOT_FOUND`**: this host mounts SYSTEM, SECURITY and SAM, but **no SOFTWARE hive**. So
`GetProfilesDirectoryW` fails (`profile.c:1599`, `Error: 2`), `LoadUserProfileW` fails
(`profile.c:2077`, then `misc.c:451`), `HandleLogon` takes `goto cleanup` (`sas.c:628`) and winlogon
returns to the logon screen. The boot quiesces at its normal SAS milestone — **no crash park, and
`userinit.exe` was NOT spawned.**

Already in place for the next batch: the genuine ReactOS `software` hive (471040 B, contains
`ProfileList\ProfilesDirectory`) is in the fetched tree, and `userinit.exe` (300544 B) is already on
the image at `\reactos\system32\userinit.exe` via the recursive full-FS copy. The mount mechanism is
the 3-hive `KeyRef`-top-nibble scheme (`base_hive()`), which needs a 4th slot. Downstream:
`CreateUserEnvironment` -> `SetDefaultLanguage` -> `AllowAccessOnSession` -> `StartUserShell` ->
`CreateProcessAsUserW(userinit.exe)` as a real 6th hosted process under the logon token.

## One spec re-pointed, honestly

`exec_general_nt_create_thread` sampled the LIVE `WL_LISTENER_TCB`, which the real thread-termination
mechanism zeroes — and now legitimately does: past the logon winlogon's transient workers (tids
20/21) self-terminate, so the post-quiesce dead-client injection reaches its last expendable
candidate, the RPC listener, and reclaims its TCB before the gate. That clause was measuring the
...
## Batch 61 — the TEB-clobber family was ONE missing kernel step: `KeGdiFlushUserBatch` (gate 252 -> **253/99, ZERO FAILs**)

**★ THE WALL WAS NOT `RtlEnterCriticalSection`, AND THE DISASSEMBLY SAID SO IMMEDIATELY.**
`ntdll+0x13284` is `RtlEnterCriticalSection+0x14 = lock incl 0x8(%rcx)` — the FIRST dereference of
the critical section. `DebugInfo`, `LockSemaphore`, the `-1` NO_DEBUG_INFO sentinel and our
`Enter`/`Leave` logic are all downstream of an instruction that never executed. A GPR dump at the
fault (`[cs-diag]`, new) gave `rcx = rax = 0x0005_0005_0018_010F` — **non-canonical**, which is
exactly what turns a memory operand into `#GP(0)` instead of `#PF` — and `ret@sp+0x38 =
rpcrt4+0x4d97e` = `RPCRT4_SetThreadCurrentConnection`, whose `get_or_create_threaddata()` returns
`NtCurrentTeb()->ReservedForNtRpc` (x64 **TEB+0x1698**) without validating it and enters
`tdata->cs` at `tdata+0x10`.

**The slot was WRITTEN CORRECTLY and then EATEN.** An `[ntrpc]` watch on those 8 bytes, sampled at
every service-loop event and on both sides of every win32k dispatch, showed
`0 -> 0x0000_0100_3003_a240` (rpcrt4's own `HeapAlloc`'d `threaddata`, stored by `rpcrt4+0x5001f`)
and then three successive corruptions, with `TEB+0x1680 = 0xff`,
`TEB+0x1690 = 0xffff00c8_d0d40000` and `TEB+0x16a0 = 0x8000` appearing beside it.

**ROOT CAUSE — `gdi32`'s deferred GDI batch, unbounded because the KERNEL never flushed it.**
`GdiAllocBatchCommand` (`gdi32p.h:406`) appends each deferred command at
`TEB + 0x300 + GdiTebBatch.Offset`; its overflow guard calls `NtGdiFlush()` and appends **anyway**,
because the reset is the kernel's job: `KiSystemCallHandler` (`ntoskrnl/ke/amd64/traphandler.c:180`)
calls `KeGdiFlushUserBatch()` before EVERY win32k system call when `GdiBatchCount != 0`, and
`NtGdiFlushUserBatch` (`gdibatch.c:487`) ends with `Offset = 0; GdiBatchCount = 0; HDC = 0`. Our
executive plays `KiSystemCallHandler` and never did it, so `Offset` reached **0x15BA (11.8x
`GDIBATCHBUFSIZE`)** and GDI records marched through `Win32ClientInfo` (0x800),
`StaticUnicodeString` (0x1258), the TLS slots (0x1480) and `ReservedForNtRpc` (0x1698). **This is
the single cause of batch 53's ACS clobber, batch 59/60's `StaticUnicodeString` clobber and this
`#GP`** — and why win32k measured innocent every time (`ro-maps = 0`, `store-faults = 0`).

**FIX.** `ke_gdi_flush_user_batch(pi, client_teb)` at the win32k syscall entry, clearing
`Offset`/`GdiBatchCount` through the executive's OWN aliases of the client's TEB pages (never
win32k's view, so `exec_teb_not_clobbered_by_win32k` keeps its meaning). Ship switch
`GDI_USER_BATCH_FLUSH`. No `rust-micro` change.

**THE COST OF BOUNDING `Offset`, MEASURED, AND WHAT IT FORCED.** `ExtTextOutW` (`text.c:603`) and
`PolyPatBlt` (`painting.c:655`) fall through to the real win32k call **only when the record would not
fit** — the runaway `Offset` was what made them reach win32k. Bounded, they get batched, and this
host does not execute batch records: `exec_msgina_credentials_entered`'s read-back went 1 -> 0. The
data did not move, only its transport, so the flush WALKS the records it clears
(`nt_user_callback::walk_gdi_batch`, pure + 5 host tests) and reads the `GdiBCTextOut` record's
inline string. `GdiTebBatch.HDC` is deliberately left claimed (`gdi32p.h:443` then refuses every
other DC, so gdi32 issues real system calls). A control experiment that disabled batching outright
via a non-HDC sentinel was **measured and rejected: 226/99, 27 FAILs**.

**THE SECOND `\pipe\lsarpc` WORKER: an EMPTY ETHREAD POOL, not a missing slot.** Past the `#GP`
winlogon really binds `\pipe\lsarpc`; rpcrt4's second `RPCRT4_new_client` create still failed until
`[thread-pool] REFUSED NtCreateThread pi=4 used-mask=0x1f slots=5` named it.
`PM_RUNTIME_THREAD_SLOTS` 5 -> 8; the RPC completes and the deadman never trips.

**NEW SPEC** `exec_gdi_user_batch_flushed` (flushes >= 1 on the real path, `Offset` never past
`GDIBATCHBUFSIZE`, live `ReservedForNtRpc` canonical, `TEB+0x1680/0x1690/0x16a0` untouched, canary
intact). **BYPASS**: 253 -> **252/99, exactly 1 FAIL**, `live-Offset = 0x15BA`, the clobber and the
`#GP` both back.

**NEXT.** `LoadUserProfileW` reaches `profile.c:1094`'s `RegLoadKeyW(HKEY_USERS, <SID>, ntuser.dat)`
and gets `Error: 2` — the copied profile has no `ntuser.dat`. `NtLoadKey` (102) is never reached and
still unimplemented; no hive is loaded; `userinit.exe` did NOT spawn. Order: (1) stage
`config\default` AS the profile's `ntuser.dat`, (2) implement `NtLoadKey`/`NtUnloadKey` +
`\Registry\User` (widen `HIVE_SEL_MASK` to `0xF000_0000`), (3) `CreateUserEnvironment` ->
`WlxActivateUserShell` -> `CreateProcessAsUserW(userinit.exe)` as a 6th hosted process — cslots are
at 82.8 % (107883/130357), so slot recycling may be needed first.
