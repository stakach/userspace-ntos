...
The kernel32+0x7167e NULL deref is **GONE**. winlogon now runs PAST kernel32's codepage init: new post-map SSNs `4:113 4:125 4:185 4:27` (map / NtOpenKey / NtQuerySystemInformation / NtClose) and its demand-fault count jumps **140 → 173 pages**. It now parks at **ntdll+0x4f22 = `RtlRaiseException`+2** (`[sec-stop] badge=4 (winlogon) label=4 m0=…804f22 m1=3 exc#=0`), i.e. RIGHT AFTER the `int3` in **OUR `RtlRaiseException` stub** (RVA 0x4f20 = `push rax; int3; pop rax; ret`). So winlogon reached its own code that RAISES an exception (an SEH-based control path in its init/WinMain), and our `RtlRaiseException` is an honest `int3` seam — it does NOT build a CONTEXT + run `RtlDispatchException`/unwind to a handler. Gate 150 held (identical 150 PASS / 27 FAIL vs BATCH 12; zero spec diff). **NEXT FRONTIER = make `RtlRaiseException` REAL on target: capture a CONTEXT, build the EXCEPTION_RECORD, and drive the existing SEH machinery (`nt-ntdll::rtl::exception::RtlDispatchException` + `nt-ntdll::ki`) to find + invoke the `__except` handler (or `RtlUnwind` to it) — the `.pdata`/function-table registry already exists (2b/2c). Then winlogon's exception is caught and it continues its real WinMain → StartServicesManager/StartLsass → SwitchDesktop → the 0x003a6ea5 paint.** (This is the SEH dispatch seam flagged as target-only since 2c — now the live frontier, a proper new batch.)
