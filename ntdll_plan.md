...
### How far winlogon got + the NEW wall
`[dbg] PebLdr` prints per process: **smss n=2** (image+ntdll), **csrss n=3**, **winlogon n=33** (image+ntdll+31 cascaded/delay DLLs). The **kernel32+0xff13 GetModuleFileNameW NULL-Flink wall is GONE** (no `0xff13` anywhere in the boot log). winlogon advances PAST kernel32's post-loader + user32 init and now parks in **`gdi32+0x3f0cc`** (`[batch10]` RIP samples = 0x8012f0cc; gdi32 mapped @0x800f0000, in PEB->Ldr) — a real gdi32 **process-attach / GdiProcessSetup**-region frontier, well past the loader. **Gate held identical 150 PASS / 98 FAIL** (zero spec diff), kernel specs "All specs passed!", host green (nt-ntdll 165). **NEXT FRONTIER: gdi32+0x3f0cc — winlogon's gdi32 process-attach (likely GdiDllInitialize→GdiProcessSetup→NtGdiInit / a win32k GDI-connect the executive doesn't yet service, or a gdi32 shared-handle-table read). Diagnose-first (disasm the fault @gdi32+0x3f0cc + the .pdata fn + retaddr chain) then service the missing gdi32↔win32k seam.** Remaining path to paint: gdi32 attach → winlogon WinMain → InitializeSAS → StartServicesManager (services.exe on our ntdll) + StartLsass (lsass on our ntdll) → SwitchDesktop → the 0x003a6ea5 desktop paint.
