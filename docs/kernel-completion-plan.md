...
- `[~]` B3: Bind PnP devnodes to driver services from registry `Enum`/`Services` data and let
  ...
  registry-devnode path. The boot-framebuffer video bridge now shares host-tested video-port IOCTL
  semantics between the direct win32k EngDeviceIoControl shim and the I/O Manager backend. Its
  `\Driver\<service>`, `\Device\Video<N>`, and DeviceMap identity strings are generated by the
  host-tested video crate and stored as component-visible metadata instead of being spelled as
  executive globals. The real videoprt/miniport route now has registry-selected demand-start
  setup state, optional-interrupt PCI resources, direct win32k `EngDeviceIoControl` routing,
  resource-shaped Bochs VRAM reporting, and caller-address-space framebuffer mapping through the
  hosted `VideoPortMapMemory` path. Loaded GDI/display driver image records also grow in
  component-visible win32k pool storage, so win32k's hosted trampolines never depend on
  executive-only heap pointers. The temporary bridge still publishes one boot-framebuffer-backed
  device; remaining display debt is retiring or reclassifying that bridge now that the hosted
  `bochsmp.sys`/`framebuf.dll` route reaches Explorer shell chrome.
...
1. Continue B3 cleanup after the NDIS-backed PCI path for ReactOS `e1000.sys`: generated SYSTEM hive
  ...
  through PnP/start paths. The temporary boot-video bridge now consumes a selected video object
  number and host-tested video identity materialization for its driver path, device path, and
  DeviceMap publication. Remaining display debt is hosting real videoprt/miniport instead of the
