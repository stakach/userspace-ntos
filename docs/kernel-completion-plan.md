...
- `[~]` B3: Bind PnP devnodes to driver services from registry `Enum`/`Services` data and let
  ...
  setup state and optional-interrupt PCI resources; remaining display debt is validating the
  hosted route in a serialized boot, then closing caller-address-space framebuffer mapping before
  retiring the boot-framebuffer bridge. Loaded GDI/display driver image records also grow in component-visible
  win32k pool storage, so win32k's hosted trampolines never depend on executive-only heap pointers.
  The temporary bridge still publishes one boot-framebuffer-backed device, but its registry-selected
  driver/service metadata is now component-visible owned data rather than 32/128 byte static buffers.
