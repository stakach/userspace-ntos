...
- `[~]` B3: Bind PnP devnodes to driver services from registry `Enum`/`Services` data and let
  ...
  executive globals. Remaining display debt is hosting real videoprt/miniport instead of the
  boot-framebuffer bridge. Loaded GDI/display driver image records also grow in component-visible
