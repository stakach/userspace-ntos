...
A4/B3 win32k paging seen-set scaling cleanup (2026-08-21): win32k's client paging hierarchy seen set
now grows in a vector instead of silently stopping at an 8192-entry static array. The set still
records level-tagged PDPT/PD/PT keys, rejects duplicates, and forgets stale hierarchy keys before
repair. Allocation failure now emits an explicit `[w32paging]` diagnostic rather than pretending the
key was recorded. Local validation is green for `cargo fmt --all`, executive
`cargo check --manifest-path components/ntos-executive/Cargo.toml --target
x86_64-unknown-none`, and `git diff --check`. Serialized desktop proof
`.tmp/run-headless-w32-seen-vec-20260821.log` reaches Explorer shell chrome with `293/293` checks
passing, keeps `exec_win32k_desktop_painted`, `exec_msgina_modal_paint_prefix`,
`exec_explorer_user_callbacks_redirected`, and `exec_explorer_wndproc_installed_by_client` green,
reports `ucb-redirects=906 ucb-returns=906`, and shows no `[w32paging]` allocation failures. Review
...
A4/B3 keyed-event waiter scaling cleanup (2026-08-22): `NtWaitForKeyedEvent` and
`NtReleaseKeyedEvent` rendezvous waiters no longer use fixed 16-slot parallel arrays. Both wait-side
and release-side parks now use typed vector-backed `KeyedWaiterTable` records that preserve the
stolen reply cap, TID, deadline, and syscall resume context together, reuse cleared rows, hard-fail
service-loop startup if the initial reserve cannot be provisioned, and report allocation/store
failures. The pool census reports
`keyed-wait=<wait live>/<records>/<cap>/<alloc-fail>/<store-fail>:<release live>/<records>/<cap>/<alloc-fail>/<store-fail>`.
Local validation is green for `cargo fmt --all`, executive
`cargo check --manifest-path components/ntos-executive/Cargo.toml --target x86_64-unknown-none`,
`git diff --check`, and `git -C rust-micro diff --check`. Serialized desktop proof
`.tmp/run-desktop-keyed-waiters-vec-20260822.log` reaches genuine userinit/Explorer launch and
Explorer shell chrome with `293/293` checks passing, keeps `exec_teb_not_clobbered_by_win32k`,
`exec_msgina_credential_keystrokes_delivered`, and `exec_explorer_shell_chrome_painted` green,
...
A4/B3 pipe-name waiter scaling cleanup (2026-08-22): root `FSCTL_PIPE_WAIT` waiter bookkeeping no
longer exposes a const-generic table identity sized from `WAIT_REPLY_POOL_N - 1`. `nt-io-manager`
now owns a non-generic growable `PipeNameWaiterTable` with explicit reset, live/record/capacity
stats, and allocation/store failure counters. The executive pre-reserves the durable root-wait queue
before the service-loop heap rewind mark, marks later successful growth through the shared
wait-table dirty bit, and reports
`pipe-name-wait=<live>/<records>/<cap>/<alloc-fail>/<store-fail>` in pool census. Local validation is
green for `cargo fmt --all`, `cargo test -p nt-io-manager pipe_name_waiter -- --nocapture`,
`cargo test -p nt-io-manager`, executive
`cargo check --manifest-path components/ntos-executive/Cargo.toml --target x86_64-unknown-none`,
`git diff --check`, and `git -C rust-micro diff --check`. Serialized desktop proof
`.tmp/run-desktop-pipe-name-waiters-vec-20260822.log` reaches genuine userinit/Explorer launch and
Explorer shell chrome with `293/293` checks passing, keeps
`exec_teb_not_clobbered_by_win32k`, `exec_msgina_credential_keystrokes_delivered`,
`exec_profile_ntuser_dat_present`, and `exec_explorer_shell_chrome_painted` green, reports
`pipe-name-wait=0/0/808/0/0`, and commits writable profile state with
`[writable-fs-snapshot] committed generation=5 bytes=822142`. Review adjustment: root NPFS wait
...
image table with bounded support-driver records and ordered provider-closure planning. The hosted
Bochs/videoprt route remains the only display route: it publishes
...
- B3 videoprt boundary slice (2026-08-16). `nt-video-miniport` now models the NT5 video-port split
  directly: `VideoPort` owns port-handled controls such as `IOCTL_VIDEO_INIT_WIN32K_CALLBACKS` and
  `IOCTL_VIDEO_UNMAP_VIDEO_MEMORY`, while `VideoMiniportAdapter` receives the forwarded
  mode/memory IOCTLs that ReactOS `videoprt.sys` would translate into a miniport `HwStartIO` request.
  The existing boot-framebuffer adapter remains the first implementation, but the executive no longer
  calls boot-video-specific dispatch helpers from beside the I/O Manager path; both the projected
  `DriverDispatchBackend` and the direct `EngDeviceIoControl` shim go through the same video-port
  wrapper. `nt-video-miniport` also now owns host-tested video identity materialization for
  `\Driver\<service>`, `\Device\Video<N>`, and the `HARDWARE\DEVICEMAP\VIDEO` value names/data, so
  the executive publishes and opens the same generated path instead of retaining local `Video0`
  constants. The display registry selection and win32k display-registry import shims now use
  growable `Vec` storage for registry strings, display DLL paths, value names/data, and handles,
...
- B3 demand-start display miniport selection slice (2026-08-16). Generated SYSTEM-hive setup state
  now installs the staged ReactOS Bochs display miniport from `bochsmp.inf` instead of relying on a
  kernel-side display identity: the host generator parses the INF model, service, and AddReg data,
  emits `Services\\bochsmp`, a PCI `Enum` devnode for `PCI\\VEN_1234&DEV_1111`, the display class
  key, and `Services\\bochsmp\\Device0` with `InstalledDisplayDrivers=framebuf`. Config Manager now
  exposes demand-start PnP bindings, and the executive builds a separate demand-start config PnP
  launch plan while feeding boot/system and demand-start plans through the same hosted PCI grant,
  resource-window publication, AddDevice, and START_DEVICE path. PCI resource assignment now models
  optional interrupts and optional DMA: network/storage drivers still require DMA grants, while the
  Bochs display adapter receives MMIO plus its real legacy DISPI I/O ports `0x1CE..0x1CF` without a
  fake IRQ or common buffer. The storage shared hive window was expanded to hold the larger installed
  hardware registry image. Validation: `cargo fmt --all`, `cargo test -p nt-config-manager`,
  `cargo test -p nt-cm-resources`, `cargo test -p nt-pnp`, `cargo test -p nt-hive-core
  generated_hive -- --nocapture`, `cargo test -p nt-video-miniport -- --nocapture`, and
  `cargo check --manifest-path components/ntos-executive/Cargo.toml --target
  x86_64-unknown-none`. Review adjustment: run a serialized `./run.sh --desktop` next and inspect
...
- B3 boot-video identity separation (2026-08-16). The follow-up serialized desktop run
  proved the storage-window correction restored config-hive decode and registry-selected PCI
  resources, but `bochsmp.sys` still failed the B3 PnP proof after `VideoPortInitialize`: the
  temporary boot-framebuffer Video0 bridge had registered its I/O Manager driver as
  `\\Driver\\bochsmp`, so the later real hosted miniport could not register the same canonical
  driver object and `load_driver` collapsed to `None`. The boot bridge now uses a distinct
  `\\Driver\\bootvid` identity while keeping the selected display service's `Device0` registry
  metadata intact; the real registry-selected miniport remains the only owner of `\\Driver\\bochsmp`.
  Driver-registration failures now print their exact NTSTATUS before returning `None`, so future
  route collisions are visible at the source. Validation continued in the next hosted-video slice.

- B3 hosted Bochs/framebuf route proof (2026-08-16). Serialized desktop validation
  `RUN_LOG=.tmp/run-desktop-b3-video-vram-cap.log ./run.sh --desktop` proves win32k no longer falls
  back to the boot-framebuffer IOCTL implementation for the display driver path. Hosted win32k binds
  `EngDeviceIoControl` directly to the executive video-device RPC, the real `bochsmp.sys` miniport
  handles `QUERY_NUM_AVAIL_MODES`, `QUERY_AVAIL_MODES`, `SET_CURRENT_MODE`, and
  `MAP_VIDEO_MEMORY`, and the video-port I/O-port shim reports Bochs VRAM from the assigned
  framebuffer resource instead of QEMU's full adapter VRAM. The proof log shows the mode list reduced
  from the earlier 1520-byte oversized list to `bytes=320`, `VideoPortMapMemory` mapping
  `phys=0x80000000 len=3145728` to `0x100009000000`, `IOCTL_VIDEO_MAP_VIDEO_MEMORY` returning
  success, and `PDEVOBJ_lChangeDisplaySettings status=0`. The same run reaches genuine
  `userinit.exe`, `explorer.exe`, and `kbswitch.exe` execution, and the host screenshot
  `.tmp/run-desktop-b3-video-vram-cap-visible.png` shows Explorer shell chrome rendered through the
  hosted Bochs/framebuf route. Review adjustment: B3's display frontier is now retiring or
  reclassifying the temporary boot-framebuffer bridge and reducing diagnostic traces once the
  hosted route has a stable automated shell-pixel gate.

- B3 boot-framebuffer route retirement (2026-08-16). The executive no longer pre-publishes a
  boot-framebuffer-backed Video0 route or keeps a boot-only video driver/device/file backend alive
  beside the hosted PnP stack. `publish_display_device_route`,
  `publish_boot_framebuffer_video_device`, the boot-video route backend, and the boot-only
  driver/device registration helpers were removed; `record_display_driver` now fails closed unless a
  hosted videoprt route is ready. Serialized desktop validation
  `.tmp/run-desktop-b3-no-boot-video-route.log` shows no early display-route publication, the hosted
  Bochs route publishing before win32k probes `HARDWARE\\DEVICEMAP\\VIDEO`, direct `\\Device\\Video0`
  opens succeeding through the hosted I/O Manager route, mode enumeration returning `bytes=320`,
  `VideoPortMapMemory` mapping `phys=0x80000000 len=3145728` to `WIN32K_FB_VA`, and
  `PDEVOBJ_lChangeDisplaySettings status=0`. The same run reaches genuine `userinit.exe` and
  `explorer.exe`, reports `[explorer-paint] begin/end=5/20`, saturates the framebuffer proof with
  `786432/786432` non-background pixels, and passes `exec_explorer_shell_chrome_painted` with
...
- B3 hosted support-driver record cleanup (2026-08-16). Hosted dependency initialization no longer
  ...
  `exec_explorer_shell_chrome_painted` green. Review adjustment: the next dependency-frontier cleanup
  is the ordered provider closure captured in the following B3 entry.

- B3 hosted dependency closure cleanup (2026-08-16). Hosted dependency discovery now builds an
  ordered provider closure instead of loading only the primary driver's direct `.sys` imports. The
  planner reads each supported provider's raw PE imports from the ReactOS driver directory, rejects
  dependency cycles, de-duplicates shared providers case-insensitively, enforces the shared support
  record capacity, and emits dependencies in postorder so a provider's own imported `.sys` images are
  already mapped and registered before its IAT is snapped. The existing resolver still fails closed
  for unsupported providers, ordinal imports, and missing exports; no kernel shim or name-specific
  fallback was added. Validation: `cargo fmt --all`, executive
  `cargo check --manifest-path components/ntos-executive/Cargo.toml --target
  x86_64-unknown-none`, `git diff --check`, and serialized desktop proof
  `.tmp/run-desktop-dependency-closure.log`, which reaches `296/296` with
  `dependency ndis.sys -> reactos\system32\drivers\ndis.sys`, `support_count=1`,
  `exec_generic_pci_support_driver_entry`, `exec_gdi_user_batch_flushed`, and
  `exec_explorer_shell_chrome_painted` green. Review adjustment: B3's active non-display frontier is
...
- NPFS retained-IRP cancellation in progress. The old thread-teardown path no longer detaches
  ...
  component-plane reply when `KeSetEvent`, `KePulseEvent`, or `KeReleaseSemaphore` makes the raw
  `KEVENT`/`KSEMAPHORE` header ready. Pulse is handled as set-wake-clear at the executive boundary,
  so parked waiters observe the pulse while the final inline event state remains clear. Zero-timeout
  polls still return `STATUS_TIMEOUT`; finite timed waits remain fail-closed `STATUS_NOT_IMPLEMENTED`
  until a driver-owned timer queue exists.
