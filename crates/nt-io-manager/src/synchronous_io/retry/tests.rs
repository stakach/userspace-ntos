...
fn waiter(file: u64, tid: u64) -> SynchronousFileWaiter {
    let mut waiter = SynchronousFileWaiter::waiting(
        file, 7, 9, 0x40, 3, 191, 2, tid, tid + 100, true, true,
        0x1000, 0x1002, 0x2000, 0x202,
    );
...
fn promoted(table: &mut SynchronousFileWaitTable, file: u64, tid: u64)
    -> SynchronousFileRetryIdentity
{
    let slot = table.park(waiter(file, tid)).unwrap();
    table.promote_exact(slot, file, tid).unwrap();
    table.retry_identity(slot, file, tid).unwrap()
}
...
fn assert_retained(table: &mut SynchronousFileWaitTable, identity: SynchronousFileRetryIdentity) {
    ...
    assert!(table.has_retry_delivery_for_file(original.file_id));
    ...
    assert!(!table.has_retry_delivery_for_file(original.file_id + 1000));
    ...
    assert!(table.take_exact(identity.slot, original.file_id, original.tid).is_none());
    assert!(table.take_alertable_waiting_exact(identity.slot, original.file_id, original.tid).is_none());
    assert!(table.alertable_waiting_for_thread(original.tid).is_none());
    assert_eq!(table.take_thread_with(original.tid, |_| panic!("delivery cannot be discarded")), 0);
    assert!(table.take_promoted(original.pi, original.tid, original.badge, original.service_number).is_none());
    assert!(!table.reset());
...
fn retry_ack_and_local_retirement_preserve_exact_arguments_grant_and_reply() {
    ...
    assert_eq!(table.next_retry_for_file(10), Some(identity));
    ...
    assert_eq!(table.next_retry_for_file(10), None);
    assert_retained(&mut table, identity);
    table.record_retry(&mut attempt, SynchronousFileRetryOutcome::Acknowledged).unwrap();
    ...
    assert_eq!(table.retry_delivery(identity).unwrap().phase,
        SynchronousFileRetryPhase::Acknowledged { local_error: Some(13) });
    assert_retained(&mut table, identity);
    ...
    assert_eq!(table.next_retry_for_file(10), None);
    assert!(table.finish_retry(identity, Ok(())).is_err());
    let consumed = table.take_promoted(original.pi, original.tid, original.badge, original.service_number).unwrap();
    assert_eq!(consumed, SynchronousFileWaiter { reply_cap: 0, ..original });
...
fn no_entry_retry_issues_fresh_ticket_and_rejects_old_or_mutated_ack() {
    ...
    table.record_retry(&mut rejected, SynchronousFileRetryOutcome::NotEntered(17)).unwrap();
    assert_eq!(table.retry_delivery(identity).unwrap().phase,
        SynchronousFileRetryPhase::Ready { last_error: Some(17) });
    assert_retained(&mut table, identity);
    let mut retry = table.begin_retry(identity).unwrap();
    assert!(retry.attempt > rejected.attempt);
    assert!(table.record_retry(&mut rejected, SynchronousFileRetryOutcome::Acknowledged).is_err());
    let mut stale = SynchronousFileRetryAttempt { consumed: false, ..rejected };
    assert!(table.record_retry(&mut stale, SynchronousFileRetryOutcome::Acknowledged).is_err());
    retry.waiter.reply_cap += 1;
    assert!(table.record_retry(&mut retry, SynchronousFileRetryOutcome::Acknowledged).is_err());
    retry.waiter.reply_cap -= 1;
    table.record_retry(&mut retry, SynchronousFileRetryOutcome::Acknowledged).unwrap();
    assert!(table.record_retry(&mut retry, SynchronousFileRetryOutcome::Acknowledged).is_err());
}
...
fn indeterminate_delivery_blocks_teardown_and_younger_file_waiter_not_other_files() {
    ...
    let mut attempt = table.begin_retry(identity).unwrap();
    table.record_retry(&mut attempt, SynchronousFileRetryOutcome::Indeterminate(29)).unwrap();
    assert_eq!(table.retry_stats().indeterminate, 1);
    assert_eq!(table.next_retry_for_file(10), None);
    assert_retained(&mut table, identity);
    assert!(table.promote_exact(younger, 10, 2).is_none());
    assert!(table.begin_retry(identity).is_err());
    assert!(table.finish_retry(identity, Ok(())).is_err());
...
fn foreign_tables_and_reused_slots_cannot_accept_old_identity_or_attempt() {
    ...
    assert_eq!(second.record_retry(&mut attempt, SynchronousFileRetryOutcome::Acknowledged),
        Err(SynchronousFileRetryError::WrongIdentity));
    assert!(second.begin_retry(a).is_err());
    assert!(second.finish_retry(a, Ok(())).is_err());
    first.record_retry(&mut attempt, SynchronousFileRetryOutcome::Acknowledged).unwrap();
    ...
    assert!(first.record_retry(&mut attempt, SynchronousFileRetryOutcome::Acknowledged).is_err());
    assert_eq!(first.next_retry_for_file(10), Some(replacement));
}
...
fn exhausted_attempt_or_admission_leaves_existing_owner_untouched() {
    ...
    assert!(matches!(table.begin_retry(identity), Err(SynchronousFileRetryError::Exhausted)));
    assert_retained(&mut table, identity);
    assert_eq!(table.retry_stats().ready, 1);
...
fn ordered_local_only_redrive_skips_failed_earlier_retirement_and_uncertain_rows() {
    ...
    table.record_retry(&mut attempt, SynchronousFileRetryOutcome::Indeterminate(9)).unwrap();
    assert_eq!(table.next_acknowledged_retry_after(None), Some(earlier));
    assert!(!table.finish_retry(earlier, Err(9)).unwrap());
    assert_eq!(table.next_acknowledged_retry_after(Some(10)), Some(later));
    assert_eq!(table.next_acknowledged_retry_after(Some(30)), None);
    assert_eq!(table.next_acknowledged_retry_after(None), Some(earlier));
    table.finish_retry(later, Ok(())).unwrap();
    assert_eq!(table.next_acknowledged_retry_after(Some(10)), None);
}
...
fn scope_queries_cover_waiting_retired_grant_and_every_retained_delivery_phase() {
    ...
    assert!(table.retry_identity(slot, 10, 1).is_none());
    table.promote_exact(slot, 10, 1).unwrap();
    let identity = table.retry_identity(slot, 10, 1).unwrap();
    assert_retained(&mut table, identity);
    let mut attempt = table.begin_retry(identity).unwrap();
    assert_retained(&mut table, identity);
    table.record_retry(&mut attempt, SynchronousFileRetryOutcome::Acknowledged).unwrap();
    ...
    assert_eq!(table.take_thread_with(1, |waiter| assert_eq!(waiter.reply_cap, 0)), 1);
    assert!(!table.has_waiter_for_pi(2));
    assert!(table.reset());
}
...
fn retired_unconsumed_grant_still_excludes_younger_file_promotion() {
    ...
    table.finish_retry(identity, Ok(())).unwrap();
    assert!(!table.has_retry_delivery_for_file(10));
    assert!(table.has_promoted_for_file(10));
    assert_eq!(table.oldest_waiting_for_file(10), None);
    assert_eq!(table.promote_exact(younger, 10, 2), None);
    table.take_promoted(2, 1, 101, 191).unwrap();
    assert!(!table.has_promoted_for_file(10));
    assert_eq!(table.oldest_waiting_for_file(10).unwrap().0, younger);
    assert!(table.promote_exact(younger, 10, 2).is_some());
}
