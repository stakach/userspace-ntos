...
impl<C> ByteRangeLockTable<C> {
    ...
    pub fn unlock_single(&mut self, request: ByteRangeLockRequest) -> u32 {
        ...
        let Some(index) = self.active.iter().position(|active| *active == request) else {
    ...
    pub fn cancel_wait(&mut self, wait_id: ByteRangeWaitId) -> bool {
        let Some(index) = self.waiting.iter().position(|waiting| waiting.id == wait_id) else {
            return false;
        };
...
mod tests {
    ...
    fn cleanup_releases_only_the_exact_file_object_and_cancels_its_waiters() {
        ...
        assert_eq!(locks.active_count(), 2);
        let completions = [locks.pop_completion().unwrap(), locks.pop_completion().unwrap()];
        assert!(completions
            .iter()
    ...
    fn zero_length_locks_do_not_conflict() {
        ...
        assert_eq!(locks.check_write(7, owner(12, 3, 0), 100, 1), STATUS_SUCCESS);
    }
}
