...
pub fn time_fields_to_time(tf: &TimeFields) -> Option<i64> {
    ...
    let secs = days
        ...
        .checked_add(tf.hour as i64 * 3600)?
        .checked_add(tf.minute as i64 * 60)?
        .checked_add(tf.second as i64)?;
    secs.checked_mul(TICKS_PER_SEC)?
        .checked_add(tf.milliseconds as i64 * TICKS_PER_MS)
}
