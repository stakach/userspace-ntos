...
mod tests {
    ...
    fn decodes_reactos_amd64_context_layout() {
        let context = amd64_context::CapturedAmd64Context::capture(
            |address, bytes| {
    ...
    fn initializes_reactos_amd64_user_context_contract() {
        ...
        let decoded = amd64_context::CapturedAmd64Context::capture(
            |address, output| {
                assert_eq!(address, 0x1000);
        ...
        let decoded = decoded.startup_projection();
    ...
    #[test]
    fn shared_apc_fault_codec_preserves_saved_registers_and_io_terminal_status() {
        use amd64_context::{prepare_user_apc, UserApcContinuation, UserApcPayload};
        let mut saved = [0u64; 20];
        for (index, slot) in saved.iter_mut().enumerate() {
            *slot = 0x1000 + index as u64;
        }
        saved[2] = 0x202;
        let mut floating_point = [0; amd64_context::LEGACY_FLOATING_POINT_BYTES];
        floating_point[..2].copy_from_slice(&0x037fu16.to_le_bytes());
        floating_point[24..28].copy_from_slice(&INITIAL_MXCSR.to_le_bytes());
        let prepared = prepare_user_apc(
            &saved,
            &floating_point,
            UserApcContinuation::Fault {
                resume_ip: 0x7fff_1234,
                resume_sp: 0x7000_ff00,
                resume_flags: 0x246,
            },
            0x1000_0000,
            UserApcPayload {
                routine: 0x1111_2222,
                normal_context: 0x3333_4444,
                system_argument1: 0x5555_6666,
                system_argument2: 0x7777_8888,
            },
            0xc000_0120,
            0x7fff_ffff_ffff,
        ).unwrap();
        let bytes = prepared.frame;
        ...
        assert_eq!(read_u64(CONTEXT_RAX_OFFSET), 0xc000_0120);
    ...
    #[test]
    fn shared_apc_fault_codec_refuses_zero_resume_coordinates() {
        use amd64_context::{prepare_user_apc, UserApcContinuation, UserApcPayload};
        let mut saved = [0u64; 20];
        saved[0] = 0x1000;
        saved[1] = 0x7000_ff00;
        ...
        floating_point[..2].copy_from_slice(&0x037fu16.to_le_bytes());
        floating_point[24..28].copy_from_slice(&INITIAL_MXCSR.to_le_bytes());
        for (resume_ip, resume_sp) in [(0, 0x7000_ff00), (0x1000, 0)] {
            assert!(prepare_user_apc(
                &saved,
                &floating_point,
                UserApcContinuation::Fault {
                    resume_ip,
