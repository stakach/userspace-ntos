...
fn fixture() -> (ProcessManager, ThreadBinding<()>, ProviderLogicalCaller) {
    ...
    let binding = ThreadBinding {
        pi: 2,
        process: ProcessIdentity { pid, generation: ProcessGeneration::Hosted(3) },
        tid: u64::from(tid),
        badge: 7,
        role: (),
        tcb: 9,
...
fn references(pm: &ProcessManager, caller: ProviderLogicalCaller) -> u32 {
    pm.process_object_delete_blockers(caller.process().pid)
        .unwrap().thread_kernel_pointer_references
}
...
fn hosted_capture_retains_the_real_thread_not_its_process_or_executor() {
    ...
    assert_ne!(owner.thread_body(), pm.process_kernel_object(caller.process().pid));
...
fn initial_system_capture_is_explicit_and_preserves_the_bootstrap_reference_floor() {
    ...
    let before = pm.process_object_delete_blockers(identity.process_id()).unwrap()
    ...
    assert_eq!(pm.process_object_delete_blockers(identity.process_id()).unwrap()
        .thread_kernel_pointer_references, before + 1);
    owner.release(&mut pm).unwrap();
    assert_eq!(pm.process_object_delete_blockers(identity.process_id()).unwrap()
        .thread_kernel_pointer_references, before);
...
fn missing_or_changed_admission_never_acquires_a_reference() {
    let (mut pm, binding, caller) = fixture();
    assert!(matches!(ProviderIrpRequestor::capture_hosted::<()>(caller, None, &mut pm),
        Err(ProviderIrpRequestorError::Caller(ProviderCallerError::MissingRuntime))));
    for variant in 0..5 {
...
fn foreign_initial_designation_and_release_do_not_touch_colliding_ps_objects() {
    ...
    assert!(matches!(ProviderIrpRequestor::capture_initial_system(
        foreign.initial_system_identity().unwrap(), &mut pm),
...
fn missing_projection_fails_before_reference_acquisition() {
    ...
    let binding = ThreadBinding {
        pi: 3,
        process: ProcessIdentity { pid, generation: ProcessGeneration::Hosted(1) },
        tid: u64::from(tid), badge: 11, role: (), tcb: 12, reservations: None,
    };
    let caller = ProviderLogicalCaller::capture(binding, pm.thread_lifetime(tid).unwrap()).unwrap();
    assert!(ProviderIrpRequestor::capture_hosted(caller, Some(binding), &mut pm).is_err());
    assert_eq!(references(&pm, caller), 0);
